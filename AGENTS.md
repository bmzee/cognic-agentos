...
- `packs/lifecycle.py` (per ADR-012 + ADR-006 — pure-functional pack-lifecycle state machine. Owns the closed-enum **13-value** `LifecycleRefusalReason` at `packs/lifecycle.py:165-179` (`lifecycle_transition_invalid_state_pair` / `…_state_unknown` / `…_kind_unknown` / `…_name_unknown` / `…_terminal_state` / `…_kind_state_combination_forbidden` / `…_double_install` / `…_revoke_already_revoked` / `…_uninstall_not_revoked_or_disabled` / `…_withdraw_post_review` / `…_approve_without_review_claim` / `…_disable_not_installed` / `…_allow_list_not_approved`) — finalised at T2 from the plan-of-record's provisional ±1 count as the transition table was enumerated. `_VALID_TRANSITIONS` legal-pair table at `:196` (10 transitions / 13 legal pairs: 7 single-from + 3 multi-from × 2). `validate_transition(*, from_state, to_state, kind, transition)` pure validator at `:398` — four keyword-only args (positional misuse is a bug-class for state machines; the keyword-only contract eliminates it at the call site). `iso_controls_for(transition)` at `:343` returns the canonical ISO 42001 control tuple from the 3-value `_KNOWN_ISO_CONTROL_CODES` set (`A.5.31` / `A.5.32` / `A.6.2.4` at `:249`); helper is the single write source for the chain row's `iso_controls` tag (storage calls it internally; callers cannot inject or override). `LifecycleTransitionRefused(reason)` at `:309` is a thin wrapper carrying ONLY the closed-enum reason — no transition_name field on the exception. Pure-functional: no I/O, no DB access; the only in-tree consumer today is `packs/storage.py:108` — T3 storage's `transition()` derives the canonical ISO 42001 control tuple via `iso_controls_for(transition)` at `storage.py:456` BEFORE the `DecisionHistoryStore.append_with_precondition` precondition closure is defined (pure-functional helper call, no I/O); state validation via `validate_transition(...)` at `storage.py:480-485` runs INSIDE the closure under the row-locked view; the precondition raises `LifecycleTransitionRefused` at `storage.py:487`, `engine.begin()` at `core/decision_history.py:482` rolls back the transaction on the exception path, and the exception propagates up through `append_with_precondition` and `transition()` to the caller — neither storage layer catches it. Sprint-7B.2 portal handlers are the planned next consumer. Doctrine Lock C — the closed-enum vocabulary IS the consumer-API wire-protocol contract for `LifecycleTransitionRefused.reason`; chain audit events themselves use the `pack.lifecycle.<target_state>` namespace built at `storage.py:508` (keyed by target state, not by transition name and not by a `.refused` suffix — refusals roll back BEFORE any chain row is written, so there is no `pack.lifecycle.refused` event).)
- `packs/storage.py` (per ADR-012 + ADR-009 — Postgres-backed `PackRecordStore` at `packs/storage.py:296`; the `DecisionHistoryStore.append_with_precondition` consumer that drives every lifecycle transition through the Sprint-2.5 T2 atomic primitive. `transition()` at `:360` first runs the preflight transition-name guard at `:437-438` — `if transition not in _TRANSITION_TO_TARGET_STATE: raise LifecycleTransitionRefused("lifecycle_transition_name_unknown")` — which fail-loud refuses out-of-vocabulary transitions BEFORE any helper invocation or DB work (mirrors the asymmetric-runtime-guard fix at `packs/lifecycle.py:393-394`). On the green path: resolves `target_state = _TRANSITION_TO_TARGET_STATE[transition]` at `:440`, then derives the canonical ISO 42001 control tuple via `iso_controls_for(transition)` at `:456` BEFORE the closure is defined (pure-functional helper call; no I/O); then opens a chain-head `FOR UPDATE` lock (via `append_with_precondition`'s outer envelope). INSIDE the `_precondition` closure at `:458`: `SELECT ... FOR UPDATE` on the `packs` row at `:467-473` → `validate_transition(from_state=…, to_state=…, kind=…, transition=…)` at `:480-485` under the row-locked view → on success `UPDATE packs SET state, last_actor, updated_at` at `:494-502` under the same lock (three columns; no `version_counter` field — atomicity comes from the chain-head + row lock pair, not a counter) → closure returns the captured `(from_state, kind)` tuple. OUTSIDE the closure: `_build_record` at `:505` mints a `DecisionRecord` with `decision_type=f"pack.lifecycle.{target_state}"` at `:508` and the pre-derived `canonical_iso_controls` value (no fresh `iso_controls_for` call from `_build_record`). `append_with_precondition` at `:524` commits chain row + state-cache UPDATE + chain-head UPDATE atomically (single transaction, single commit point). Two-class refusal taxonomy (deliberately split — callers dispatch on class to distinguish state-machine refusals from API-contract refusals): `PackNotFound` at `:219` for missing-pack lookups; `PackRecordRefusalReason` at `:235` is a 1-value Literal (`pack_record_save_draft_initial_state_not_draft`) carried by `PackRecordRefused` at `:238-265` — refusal class for the `save_draft` API-contract guard only (genesis-state must be `"draft"`, never lifecycle-table refusals). Transition-table refusals raise `LifecycleTransitionRefused(reason)` from inside the precondition closure at `:487` so the engine's transactional rollback fires (no chain row, no state-cache mutation, no orphan INSERT). Doctrine Lock D — atomic chain-insert + state-cache UPDATE + chain-head UPDATE under a single transaction; integration tests pin row-lock serialisation on live Postgres + Oracle (skipped without `COGNIC_RUN_POSTGRES_INTEGRATION=1` / `COGNIC_RUN_ORACLE_INTEGRATION=1`).)
