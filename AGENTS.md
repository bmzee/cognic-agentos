...
- `packs/lifecycle.py` (per ADR-012 + ADR-006 — pure-functional pack-lifecycle state machine. Owns the closed-enum **13-value** `LifecycleRefusalReason` at `packs/lifecycle.py:175-189` (`lifecycle_transition_invalid_state_pair` / `…_state_unknown` / `…_kind_unknown` / `…_name_unknown` / `…_terminal_state` / `…_kind_state_combination_forbidden` / `…_double_install` / `…_revoke_already_revoked` / `…_uninstall_not_revoked_or_disabled` / `…_withdraw_post_review` / `…_approve_without_review_claim` / `…_disable_not_installed` / `…_allow_list_not_approved`) — finalised at T2 from the plan-of-record's provisional ±1 count as the transition table was enumerated. `_VALID_TRANSITIONS` legal-pair table at `:210` (11 transitions / 14 legal pairs: 7 single-from + 3 multi-from × 2 + cancel_draft single-from × 1; Sprint 7B.2 T4 extended from 10/13 by adding `cancel_draft` per ADR-012 §59). `validate_transition(*, from_state, to_state, kind, transition)` pure validator at `:421` — four keyword-only args (positional misuse is a bug-class for state machines; the keyword-only contract eliminates it at the call site). `iso_controls_for(transition)` at `:366` returns the canonical ISO 42001 control tuple from the 3-value `_KNOWN_ISO_CONTROL_CODES` set (`A.5.31` / `A.5.32` / `A.6.2.4` at `:264`); helper is the single write source for the chain row's `iso_controls` tag (storage calls it internally; callers cannot inject or override). `LifecycleTransitionRefused(reason)` at `:332` is a thin wrapper carrying ONLY the closed-enum reason — no transition_name field on the exception. Pure-functional: no I/O, no DB access; the only in-tree consumer today is `packs/storage.py:117` — T3 storage's `transition()` derives the canonical ISO 42001 control tuple via `iso_controls_for(transition)` at `storage.py:733` BEFORE the `DecisionHistoryStore.append_with_precondition` precondition closure is defined (pure-functional helper call, no I/O); state validation via `validate_transition(...)` at `storage.py:757-762` runs INSIDE the closure under the row-locked view; the precondition raises `LifecycleTransitionRefused` at `storage.py:764`, `engine.begin()` at `core/decision_history.py:482` rolls back the transaction on the exception path, and the exception propagates up through `append_with_precondition` and `transition()` to the caller — neither storage layer catches it. Sprint-7B.2 portal handlers are the planned next consumer. Doctrine Lock C — the closed-enum vocabulary IS the consumer-API wire-protocol contract for `LifecycleTransitionRefused.reason`; chain audit events themselves use the `pack.lifecycle.<target_state>` namespace built at `storage.py:803` (keyed by target state, not by transition name and not by a `.refused` suffix — refusals roll back BEFORE any chain row is written, so there is no `pack.lifecycle.refused` event).)
- `packs/storage.py` (per ADR-012 + ADR-009 — Postgres-backed `PackRecordStore` at `packs/storage.py:379`; the `DecisionHistoryStore.append_with_precondition` consumer that drives every lifecycle transition through the Sprint-2.5 T2 atomic primitive. `transition()` at `:616` first runs the preflight transition-name guard at `:714-715` — `if transition not in _TRANSITION_TO_TARGET_STATE: raise LifecycleTransitionRefused("lifecycle_transition_name_unknown")` — which fail-loud refuses out-of-vocabulary transitions BEFORE any helper invocation or DB work (mirrors the asymmetric-runtime-guard fix at `packs/lifecycle.py:416-417`). On the green path: resolves `target_state = _TRANSITION_TO_TARGET_STATE[transition]` at `:717`, then derives the canonical ISO 42001 control tuple via `iso_controls_for(transition)` at `:733` BEFORE the closure is defined (pure-functional helper call; no I/O); then opens a chain-head `FOR UPDATE` lock (via `append_with_precondition`'s outer envelope). INSIDE the `_precondition` closure at `:735`: `SELECT ... FOR UPDATE` on the `packs` row at `:744-749` → `validate_transition(from_state=…, to_state=…, kind=…, transition=…)` at `:757-762` under the row-locked view → on success `UPDATE packs SET state, last_actor, updated_at` at `:771-779` under the same lock (three columns; no `version_counter` field — atomicity comes from the chain-head + row lock pair, not a counter) → closure returns the captured `(from_state, kind)` tuple. OUTSIDE the closure: `_build_record` at `:782` mints a `DecisionRecord` with `decision_type=f"pack.lifecycle.{target_state}"` at `:803` and the pre-derived `canonical_iso_controls` value (no fresh `iso_controls_for` call from `_build_record`). **Sprint 7B.2 T6 slice-2 (R24 P2 Path B + B2 user-authorized CC-ADJ) added the optional keyword-only `actor_type: str | None = None` kwarg at `:625`; when non-None it is persisted as a top-level `payload["actor_type"]` key conditionally at `:800-801` so existing call sites + every pre-T6 chain row stay byte-shape compatible (additive-only schema; storage stays a thin string passthrough — the `human | service` closed-enum lives at the rbac boundary).** `append_with_precondition` at `:811` commits chain row + state-cache UPDATE + chain-head UPDATE atomically (single transaction, single commit point). Two-class refusal taxonomy (deliberately split — callers dispatch on class to distinguish state-machine refusals from API-contract refusals): `PackNotFound` at `:266` for missing-pack lookups; `PackRecordRefusalReason` at `:285` is a 4-value Literal (`pack_record_save_draft_initial_state_not_draft` + `pack_record_update_non_draft_state` + `pack_record_update_field_not_allowed` + `pack_record_update_field_invalid_shape`; Sprint 7B.2 T4 bumped from 1 to 4 values when `update_draft()` landed) carried by `PackRecordRefused` at `:293-350` — refusal class for `save_draft` API-contract genesis-state guard AND `update_draft` API-contract refusals (non-draft-state target + field-not-allowed allow-list violation + field-invalid-shape per-field validator). Sprint 7B.2 T4 added `update_draft()` at `:443` — in-place edit of `draft`-state packs via a 4-field allow-list with atomic `UPDATE … WHERE state='draft'` precondition; no chain row emitted (mirrors `save_draft()`'s genesis-state pattern at `:396`). Transition-table refusals raise `LifecycleTransitionRefused(reason)` from inside the precondition closure at `:764` so the engine's transactional rollback fires (no chain row, no state-cache mutation, no orphan INSERT). Doctrine Lock D — atomic chain-insert + state-cache UPDATE + chain-head UPDATE under a single transaction; integration tests pin row-lock serialisation on live Postgres + Oracle (skipped without `COGNIC_RUN_POSTGRES_INTEGRATION=1` / `COGNIC_RUN_ORACLE_INTEGRATION=1`).)
