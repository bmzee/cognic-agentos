...
> **Status: governance kernel built + deployment-proven; the product packs are the forward work.** Phases 1–4 (governance primitives, MCP/A2A protocol, sandbox, sub-agent, governed memory, scheduler, runtime approval, emergency controls) and the deployment substrate (Helm, ADR-024) are landed — ~134 critical-controls modules on a 95%-line/90%-branch coverage gate, ~11k tests, live Postgres/Oracle CI lanes. The governed pack loop is proven both **in-process** (Proof 1a, PR #96) and **deployed** (Proof 1b-2, PR #103 — a real in-tree MCP tool pack invoked end-to-end through a deployed kernel on kind/Helm, with audit evidence and the load-bearing carve-out refusal). **Still forward:** Proof 2 (the same pack extracted to a *separate* repo with its own CI/sign/verify/release — the pack today is still in-tree `examples/`), the deployed *agent* loop (1b-2 proved deployed *tool* invocation, not an LLM-agent task), and the actual product packs — which by ADR-001 live in separate `cognic-tool-*` / `cognic-skill-*` / `cognic-agent-*` repos that don't exist yet. See [`docs/PROJECT_STATUS.md`](docs/PROJECT_STATUS.md) for the honest built-vs-pending ledger and [`docs/BUILD_PLAN.md`](docs/BUILD_PLAN.md) / [`docs/PROJECT_PLAN.md`](docs/PROJECT_PLAN.md) for the plan + baseline.
>
...
See [`docs/AS_BUILT_CAPABILITY_MAP.md`](docs/AS_BUILT_CAPABILITY_MAP.md) for the as-built v1 capability map (what is production-wired vs seam-only, and the reconciled forward sequence) and [`docs/BUILD_PLAN.md`](docs/BUILD_PLAN.md) for the authoritative sprint sheet — that's the source of truth for work-unit accounting; this README summary is derived from them.
