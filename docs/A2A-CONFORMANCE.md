...
2. **AgentOS profile validation** — on top of the upstream schema, AgentOS imposes a **stricter profile** suited to bank-grade deployments. Fields the spec marks "optional" but AgentOS requires populated. **Profile enforcement is split** based on threat-model role:
   ...
   - **Wave-2 auth refusal** — per the wave breakdown (Wave-1 = per-tenant pinned bearer token; Wave-2 = mTLS; Wave-3 = verifiable credentials), an inbound or outbound card whose `securitySchemes` map declares any `mtlsSecurityScheme` entry asks the caller to authenticate via mTLS, which Wave-1 transport (HTTP+bearer) cannot honor. The runtime verifier refuses such cards with `agent_card_profile_wave2_auth_required`; the closed-enum payload carries `rejected_scheme_name` + `rejected_scheme_kind="mtls_security_scheme"` for operator diagnostics. The gate fires conservatively — even a card declaring `mtls` alongside a Wave-1 scheme is refused; pack authors who want forward-compatibility must wait for Wave-2 to lift the gate. Authors who need to ship today MUST omit `mtlsSecurityScheme` from their cards entirely.

   Why this subset: each of the six gates above is a vector an attacker could exploit at registration or dispatch time. Anonymous A2A would bypass the per-tenant token gate; an unsigned card would let anyone re-write the service descriptor; an empty interface list means there's nowhere to dispatch; a top-level `url` field is a spec-violation that a malicious pack could use to redirect callers; an mTLS-declaring card would route traffic through a transport AgentOS Wave-1 cannot validate.
   ...
   Why split: the runtime gate's job is to keep the registration / dispatch surface security-critical and the closed-enum refusal vocabulary narrow. Build-time CI catches missing governance metadata for AgentOS-built packs (the cognic + bank-overlay packs). Splitting keeps the runtime closed-enum count small (11 reasons total across all 3 passes — 1 upstream + 7 profile + 3 JWS, where T14 added the Wave-2 auth refusal as the 7th profile reason), keeps the registry-side `RefusalReason` mapping 1:1, and keeps the pack-author developer experience focused (`agentos validate` reports all governance-metadata violations at once during local CI).
