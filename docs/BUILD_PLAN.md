...
**CODE-COMPLETE on `feat/sprint-10.6-workload-credential-projection`; pending operator Z3/Z4 live-proof audit + PR-merge — NOT yet `CLOSED`.** Closes the Sprint 10.1 deferred Finding #1 (minted leases landed on `session.active_leases` but never reached the workload) per ADR-004 §25 amendment + ADR-017. The CLOSED status + squash-merge commit hash + final branch-commit count land at merge/closeout time (T26), matching the merge-time-CLOSED pattern used for Sprint 10 (`79cfd48`) + Sprint 10.5 (`6791eec`).
...
- **Live proofs DEFERRED to operator pre-merge audit:** Z3 (real-Vault + real-Docker, single-credential happy + Path-2) at `tests/integration/sandbox/test_z3_docker_credential_projection.py` + Z4 (real-Vault + real-Kubernetes, two-credential LIFO) at `tests/integration/sandbox/test_z4_k8s_credential_projection.py` — both env-gated (`COGNIC_RUN_{DOCKER,K8S}_CREDENTIAL_PROJECTION_INTEGRATION=1`), fail-loud-when-opted-in per the Sprint 10.1 Finding #3 import contract. No live Vault / Docker / K8s in CI, so they are NOT claimed passed here; the operator runs them before merge. The CI-executable import-contract regression (`tests/unit/test_z3_z4_import_fail_loud_contract.py`, T24) pins both halves of the contract.
...
- **Phase 3 closure** is the final piece after the 10.5 split: Phase 3 closes at the **Sprint 10.6 PR-merge, contingent on the operator Z3/Z4 audit** — not marked closed by the on-branch work.
...
**Phase 3 exit:** AgentOS provides bank-grade isolation + audit-evidence-export ready for examiner + model lifecycle registry that closes the "which fine-tuned model handled which case" procurement gap, plus the ADR-022 runtime scheduler substrate that manages work admission, priority, cancellation, backpressure, and quota refusal before sub-agents arrive. Future-product hook (Cognic Forge — Wave 2 separate repo per ADR-013) can publish fine-tuned models into the registry end-to-end. **Phase 3 fully closes at the Sprint 10.6 PR-merge** (the final Phase-3 sprint after the 10.5c → 10.6 split), contingent on the operator's Z3/Z4 live-proof audit.
