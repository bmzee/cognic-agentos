...
- `db/adapters/memory_adapters.py` — in-memory implementations for tests (Postgres+SQLite-fallback for relational; in-memory dict for others) *(2026-06-12 alignment note: as-built these live at `tests/support/adapter_fixtures.py` — test-only fakes belong in test paths per the production-grade rule)*
...
- **`core/policy/__init__.py`, `core/policy/engine.py` (minimal seed; 2026-06-12 note: the 13.5 arc added `tools.rego` only — hot-reload + decision-trail remain unscheduled)** — early Rego evaluator so Sprint 4's supply-chain grade decision and Sprint 11.5's memory enforcement do not block on Sprint 13.5. Scope of this seed:
  ...
  - Loads bundles from disk only (no hot-reload yet — originally planned for 13.5; as-built it remains unscheduled); bundles read at startup; reload requires restart
  - Default bundles published in this sprint: `policies/_default/supply_chain.rego` (used by Sprint 4) — Sprint 11.5 adds `memory.rego` and `memory_purpose_matrix.rego`; later sprints added `sandbox.rego` (8A), `elicitation.rego` (7B.4), `sampling.rego` (5), `scheduler.rego` (10.5b), and `tools.rego` (13.5a) — *(2026-06-12 as-built note: `packs.rego` / `models.rego` / `subagent.rego` / `lifecycle.rego` were never created; those decision points stayed inline)*
  ...
  - *(2026-06-12 supersession: the planned 13.5 evaluator extension — hot-reload, decision-trail API, inline-check refactor — did NOT ship in the 13.5 arc and is unscheduled; see `docs/AS_BUILT_CAPABILITY_MAP.md`)*
...
**Status (2026-05-31): Sprint 11 CLOSED — 11a + 11b both merged.** 11a merged via PR #43 (squash `6a6bd5c`); 11b merged via PR #44 (squash `5bf50e3`; 10 branch commits, memo `a093810` → T10 `84cbf01`). 11a ships the pure core (T0 doc reconciliation + `subagent/_types` + `policy` + `audit` emitter + `audit_verifier`); CC gate 90→94; PR CI green (lint+test + live Oracle/Postgres). **11b** ships the integration layer — scheduler-mediated `spawn.py` (T6) + real `ParentBudgetResolver`/`PackStateInterrogator` conformers (T5) + `SubAgent` facade + thin module-level `spawn_subagent` seam (T7/T8) + `ui_events` subagent emit hooks (T9) — as a **test/DI-proven** increment per decision memo D1 (NOT a live production dispatch path); CC gate 94→97 (`spawn`/`conformers`/`_facade` at 100% line / 100% branch on fresh `--cov-branch` data); full suite `8686 passed / 0 failed`. The deferred 11b decisions are resolved in the 11b memo: **D1** no production dispatch path, **D2** thin `spawn_subagent` seam (no `harness/`, no `base_agent.py`), **D3** split budget vocab (`subagent_child_quota_zero`); plus the **A-projector** T9 decision (recursion_capped sourced from the scoped `escalation.opened` row; `core/escalation.py` untouched). **Production path remains deferred:** the scheduler is not app-wired, the real `QuotaInterrogator` + `KillSwitchInterrogator` are Sprint 13.6 (emergency carved from 13.5, 2026-06-12), the `recursion_capped` UI emission needs escalation→emitter app-wiring, and the A2A cross-pod transport is Wave 2. **The sub-agent primitive itself is CLOSED**; live production dispatch remains deliberately deferred to later app-wiring + Sprint 13.6 emergency controls (carved from 13.5, 2026-06-12) as documented above. See [closeout note](closeouts/2026-05-31-sprint-11b-subagent-integration.md).
...
**Goal:** ship `core/memory/` per ADR-019 — the governed memory API every Layer C agent uses: **7 ADR-019 core ops** (`remember / recall / recall_episodes / forget / redact / export / list_for_subject`) **+ 3 memory-block ops** (`upsert_block / read_block / list_blocks`) = **10 total**. Three tiers (`scratch` / `task` / `long_term`); default-deny for `long_term`; per-write data-class + purpose + consent enforcement; chain-linked audit; regulator-erasure pathway.

**Sprint-11.5 reconciliation (2026-05-31, see the ADR-019 amendment):** ships as **Cut-A 11.5a** (governed substrate, full TDD) + **11.5b** (regulator-core: lifecycle + emergency, valve-resolved 2026-06-01) + **11.5c** (surfaces: export + learning_surface + portal/UI). Memory **blocks** (`persona`/`user_profile`/`agent_notes`) are singleton `long_term` projections, not a fourth tier; `SubjectRef` (`human`/`agent`) is the subject vocabulary (no unscoped memory). **Strict sub-agent rule:** sub-agents are `scratch`-only; durable memory is parent-mediated. **Scratch is fail-closed** on Redis loss in Wave-1 (the Postgres fallback is 11.5b). The substrate is DI-tested, not harness-injected (primitive-done / dispatch-deferred — same posture as Sprint 11).
...
**Deliverables:**
- `core/memory/__init__.py`, `memory/api.py` — `MemoryAPI` with **ten operations** (7 ADR-019 core + 3 block); 11.5a ships the **7-op safe surface** (4 core + 3 block: `remember`/`recall`/`recall_episodes`/`list_for_subject`/`upsert_block`/`read_block`/`list_blocks`); `forget`/`redact` land in 11.5b, `export` in 11.5c. Identity is **construction-bound** (tenant/agent/actor/served-subject), never caller-supplied; **DI-tested in 11.5 — NOT harness-injected** (the harness-injection production wiring is deferred; primitive-done / dispatch-deferred, same posture as Sprint 11); direct DB access from Layer C is architecturally forbidden (architecture-discipline test enforces)
- `cli/validators/learning_surface.py` (**11.5c**) — manifest shape validator for `[tool.cognic.learning_surface]`; closed-enum vocabulary lives in `cli/_governance_vocab.py`; out-of-gate mutation declarations refuse with `learning_surface_violation` per ADR-019 governed self-improvement doctrine
...
- **`core/dlp/__init__.py`, `core/dlp/scanner.py` (minimal seed; 2026-06-12 note: the planned 13.5 expansion did not ship in the 13.5 arc and is unscheduled)** — write-time DLP scanner so memory writes have classification at registration. Scope of this seed:
  - Pluggable `DLPScanner` protocol; the Sprint-11.5 seed is a **real deterministic checksum/regex/gazetteer** recogniser (PAN/Luhn, IBAN/mod-97, SWIFT/BIC context-cued, email, phone, regulator gazetteer) over the **build-time** data-class vocabulary `customer_pii` / `payment_data` / `credentials` / `regulator_communication` (per `cli/_governance_vocab.RESTRICTED_DATA_CLASSES`; `core/memory` + `core/dlp` own local copies — no `core→cli` runtime import — drift-pinned test-only). Free-text person names, addresses, and **checksummable national IDs remain deferred** (Presidio + per-locale recogniser plugins; unscheduled as of 2026-06-12)
  ...
  - *(2026-06-12 note: post-call DLP on tool outputs, custom recogniser plugins, and per-tenant allow/deny lists did NOT ship in the 13.5 arc and are unscheduled)* — Sprint 11.5 only wires the scanner into memory writes
- `memory/consent.py` — `ConsentToken` validator; ledger event chain-linked per write/recall
- `memory/forget.py` (**11.5b**) — soft-delete with tombstone + reaper; `forget(reason="regulator_erasure")` triggers immediate purge with `memory.regulator_erasure` chain-of-custody event
- `memory/redact.py` (**11.5b**) — partial-redaction engine; old version sealed until tombstone window expires
- `memory/export.py` (**11.5c**) — Sigstore-bundled archive per ADR-016 retention rules (reuses the `protocol/supply_chain.py` Sigstore discipline; no memory-local second bundler); RBAC `memory.export.read` required
- **Memory events ride `decision_history`** (no `core/audit.py` schema change; payload-only linkage). Write vs read carry **different** payloads. **Write-side** — `memory.write` (+ 11.5b `memory.forget` / `memory.redact` / `memory.regulator_erasure`) — are `DecisionRecord`s appended via `DecisionHistoryStore.append_with_precondition`, ISO 42001 (A.7.4, A.8.2, A.8.5, A.10.2 per ADR-019) on `iso_controls`, with a redacted-value-digest (never the value) on the `memory.write` payload. **`memory.read`** is a plain `DecisionHistoryStore.append` carrying read-shaped metadata only (no value, no digest), ISO (A.7.4, A.8.2)
- **`core/emergency/__init__.py`, `core/emergency/kill_switches.py` (minimal seed; expanded in Sprint 13.6 — emergency carved from 13.5 at the 2026-06-12 reconciliation)** — Redis-backed kill-switch state so `memory.write_freeze` works at Sprint 11.5 ship time. Scope of this seed:
  - Single kill-switch class: `memory.write_freeze` (per-tenant); checked by `memory/api.remember()` before every write
  - Redis key format reused by Sprint 13.6 (no schema migration when full kill-switch + quotas land)
  - **Fail-closed identical to the Sprint 13.6 final**: Redis unreachable → memory writes refused after ≤60s of cached-state grace
  - Sprint 13.6 extends with the other kill-switch classes (`pack`, `tool`, `model`, `tenant_packs`, `tenant_full`, `cloud`, `feature`) + quotas + portal API + full RBAC scope set. Sprint 11.5 ships only `emergency.kill.memory_write_freeze` RBAC scope; the rest are deferred
- `policies/_default/memory.rego` + `policies/_default/memory_purpose_matrix.rego` — **ship default-deny per ADR-019** (long-term writes refused, cross-subject recall refused, durable [task/long_term] restricted-data-class writes refused — `scratch` is DLP-only per §7.1 step 3b). Tenant override via local Rego layer is the only way to permit. *(2026-06-12 note: the planned 13.5 bundle expansion did not ship — the 13.5 arc deliberately left `memory.rego` untouched per the ADR-019 13.5c3 amendment; any future expansion must NOT relax the defaults.)* **Never stub permissive — that would silently authorise long-term memory writes the moment Sprint 11.5 ships.** Reuses the Sprint 4 policy-engine seed.
- **Alembic revision `20260531_0006_memory`** — single dialect-portable file (per the `0005` precedent; `sa.TIMESTAMP(timezone=True)`), NOT a raw `003_memory.sql` and NOT a separate `db/migrations/oracle/` mirror; the block singleton uses a per-dialect partial unique index (Postgres/SQLite `WHERE`; Oracle a function-based unique index whose `CASE`-NULL expressions exclude non-active/non-block rows) created in-file
- Portal API (**11.5c**): `GET /api/v1/memory/records?subject=...`, `POST /api/v1/memory/records/{id}/forget`, `POST /api/v1/memory/records/{id}/redact`, `POST /api/v1/memory/export` (RBAC-gated)
...
| **Sprint 11.5 — Agent memory governance** (2 wu) | New platform primitive: 7 MemoryAPI operations × 3 tiers × per-write enforcement (data-class + purpose + consent) × forget/redact/export pathways × episodic recall over decision_history × learning-surface validation × kill-switch integration × Postgres + Redis adapters + vector-store integration. 15 new tests including regulator-erasure chain-of-custody. **Realistic range: 2-3.5 wu.** Mitigation (applied): split into 11.5a substrate + 11.5b regulator-core + 11.5c surfaces. |
