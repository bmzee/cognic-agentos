...
**8B (Wave-1 KubernetesPodSandboxBackend per `project_openshift_deployment_target` + ADR-004 amendment):** **PLANNED.** Bank-production Wave-1 backend implementing the same `SandboxBackend` Protocol as `DockerSiblingSandboxBackend`. Same canonical image catalog (Sprint 8A); same 8-event audit taxonomy; same dual-container topology adapted to Pod spec; same proxy-mediated HTTP/HTTPS-only egress posture adapted via Kubernetes NetworkPolicy + sidecar proxy. Pre-flight tasks: (a) land the deferred 15-arm `_trigger_for_reason` conformance parametrize at `tests/conformance/sandbox/` (carried from Sprint 8A T2 plan §T10c) so both backends are pinned through one matrix; (b) OPA on CI lane (sandbox.rego + elicitation.rego currently SKIP without `opa` binary).
