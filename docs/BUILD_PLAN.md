...
- **Memory events ride `decision_history`** (no `core/audit.py` schema change) — `memory.write` / `memory.read` (+ 11.5b `memory.forget` / `memory.redact` / `memory.regulator_erasure`) are `DecisionRecord`s appended via `DecisionHistoryStore.append_with_precondition`, with ISO 42001 tags (A.7.4, A.8.2, A.8.5, A.10.2 per ADR-019) on `iso_controls` and a redacted-value-digest (never the value) in the payload
