...
- [ ] **M8.5-D — First bank NL-query analytical agent.**
  ...
  **Data-access ownership (ruled 2026-07-11; ADR-028 spec §0.4):** M8.5-D owns the dedicated data-access identity + credential-brokerage **design** — connection-profile + CredentialBroker ownership decided after source recon; the query-context token treated per the ruled framing — a private AgentOS JWS profile, not claimed to be an RFC 9068 OAuth access token; M8.5-D evaluates RFC 8693 for issuance/exchange, RFC 9068 only if OAuth access-token semantics are adopted, and RFC 9396 for fine-grained authorization details, with retaining + explicitly versioning the private profile permitted (IETF Transaction Tokens stay draft, tracked non-binding); the workload-identity profile (SPIFFE X.509-SVID/mTLS or bank-cloud equivalent); the Oracle adapter implemented **without credentials in AgentOS or the pack manifest** (Oracle target profile: workload identity → Vault/PAM lease or rotated proxy credential → Oracle proxy session → governed views + per-identity grants + DB-native audit (unified; SET_IDENTIFIER attribution); VPD parked as a bank-overlay extension (premise corrected in the D3 recon §4 — DBMS_RLS live-proven on both probed editions; licensing/support review remains a separate operator determination)).

  **D3 credential-brokerage evidence (2026-07-18 — PASS on `kind`):** the complete Bars A-H run exited 0 on clean AgentOS anchor `88e5cabd` with the released Oracle pack `v0.5.1`. Bar H proved file-only credential custody through ESO; exact wire-to-chain `credential_rotation_ref` equality; Oracle unified-audit attribution from `AN_AMIR` through proxy `COGNIC` to the independently recomputed SHA-256 reference of Amir's issuer-qualified Keycloak subject; live rotation with a changed reference and the original credential dead (`ORA-01017`); and the retained cross-scope `ORA-00942` backstop with the probe ledger unchanged. Operator-held 969-line log SHA-256 `4c99b9ff…19135`; full attempt ledger and honesty boundary are in `docs/VALIDATION-RESULTS.md`. **M8.5-D remains OPEN:** D4's full-stack AKS deployment and D5's released write pack plus live conversation-correlated auto-execution/system-turn proof remain forward gates.
