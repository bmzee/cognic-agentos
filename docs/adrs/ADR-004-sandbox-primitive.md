...
**Deferred (14A-A2+):** the `POST /api/v1/runs` route, backend-level `checkpoint()/wake()` for managed runs (the resumable-session API exists per §"Resumable-session API"; 14A-A2 wires it into the run loop), multi-backend (KubernetesPod), and manifest-driven policy.
