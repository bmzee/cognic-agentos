...
## Sprint 13c amendment — adversarial promotion gate (2026-06-10)

This amendment records the decisions taken when Sprint 13c closed the auto-generated red-team gate by **composing** the Sprint-13b `AdversarialVerdict` into a pack-approval decision. It supersedes two pieces of BUILD_PLAN shorthand (the standalone `evaluation/promotion_gate.py` and the `override.adversarial_gate` scope) and consumes — does not rebuild — the existing 5-gate composer, the Sprint-13a replay differ, and the Sprint-13b verdict + value-free chain row.

### Scope reconciliation (what the "promotion gate" actually is)

The BUILD_PLAN §1101 standalone `evaluation/promotion_gate.py` **does not exist and was not built.** The promotion gate is the **5-gate pack-approval composer** shipped in Sprint 7B.3 (`packs/approval_gates.compose_approval_gates`), which already composes signature · evaluation · **adversarial** · OWASP · reviewer-ack at the `under_review → approved` transition (ADR-012 §41). Gate-3 IS the adversarial gate. 13c's job was to **populate** it (nothing wrote `payload["adversarial"]` before 13c) + add the baseline-regression sub-term + map the verdict. The BUILD_PLAN §1102 `override.adversarial_gate` scope is **superseded by `pack.override.approval_gate`** (ADR-012 §110) — adversarial is an **overridable** gate (only `signature` is non-overridable); 13c adds **no new RBAC scope**.

### Submit-time, reference-based producer (no auto-run)

There is no OS-only "pack as LLM target", so 13c is **reference-based**: the author's `submit_draft` request gains two optional fields — `adversarial_run_id` + `baseline_adversarial_run_id` (`portal/api/packs/dto.py:262-263`). A new CC producer `build_adversarial_evidence` (`evaluation/adversarial/evidence.py:92`) resolves + verifies + maps the referenced 13b adversarial eval-run into a frozen snapshot, threaded onto the `pack.lifecycle.submitted` chain row via the new optional `payload_adversarial` kwarg on `PackRecordStore.transition` (`packs/storage.py:727`, mirrors `payload_conformance`). The producer runs **OUTSIDE** the storage transaction (mirroring the OWASP-conformance auto-run); `adversarial_run_id` is **optional** — omitted → no `payload["adversarial"]` → gate-3 reports the existing `adversarial_evidence_not_attached` (non-green, overrideable). **No new chain row**; the snapshot's `candidate_run_id` / `baseline_run_id` are the audit linkage back to the 13b eval-runs.

**Verification order (closed-enum, fail-closed).** "run id" means the **queryable** `persist_run` eval-run id (not a dangling verdict row): the producer checks candidate **existence FIRST** (`get_run`) → `adversarial_run_not_found`, THEN adversarial-ness (the new `EvalRunStore.load_adversarial_verdict` read seam, `evaluation/storage.py:328`) → `adversarial_run_not_adversarial`; when a baseline is supplied it checks baseline existence → baseline adversarial-ness → corpus-digest pairing, reusing the already-fetched candidate run. The 5-value `AdversarialEvidenceRefusalReason` (`evidence.py:18`) carried by `AdversarialEvidenceError` (`evidence.py:27`) is route-owned: `author_routes._ADVERSARIAL_EVIDENCE_STATUS` (`portal/api/packs/author_routes.py:100`) maps each reason → (404/400) and all five are pinned at the HTTP boundary.

**Request-time eval-store resolution.** The submit handler resolves the `EvalRunStore` from `app.state` at request time (`author_routes._resolve_eval_run_store`, `:109`) — mirroring the four eval-route `_require_decision_history_store` precedents (runtime-first, then the bare `decision_history_store`), **only when** `adversarial_run_id` is supplied. Absent store → fail-closed **503 `decision_history_unavailable`**. This needs no router/app threading, so the "forgot to thread it" bug class is unrepresentable; existing submit semantics are unchanged when no run id is referenced.

### The `payload["adversarial"]` snapshot (exact 6-key set)

`build_adversarial_evidence` returns exactly `{pass_rate, high_severity_failures, regressions, regression_evaluated, candidate_run_id, baseline_run_id}`. `pass_rate` + `high_severity_failures` come from the `AdversarialVerdict` (high-severity failures counted explicitly from `per_case`). With no baseline: `regressions=0, regression_evaluated=False, baseline_run_id=None`. With a baseline: `regression_evaluated=True, baseline_run_id=str(base_uuid)`, `regressions` from the differ. `candidate_run_id` is always `str(cand_uuid)` — it is the gate evidence pointer.

### Baseline regression (Sprint-13a reuse, errored excluded)

The baseline-regression sub-term **reuses Sprint-13a's `compute_replay_diff`** (`evaluation/replay.py`) over the two persisted eval-runs: the candidate `EvalRunResult` is reconstructed from `get_run` via the pure `_eval_run_from_get_run` (`evidence.py:38`); the baseline cases are fed straight from `get_run(baseline)["cases"]`; `regressions = diff.regressions` counts only `drift_kind == "regression"` (baseline-passed → candidate-failed). An **errored** candidate case is classified `errored`, **not** a regression (13a's `_classify` orders the errored check first), so a flaky/errored attack never counts as a baseline regression. **Absent baseline → the regression term is skipped** (`regression_evaluated=False`); a supplied baseline of a **different** `corpus_digest` is a submit-time refusal (`adversarial_baseline_corpus_digest_mismatch`), and a baseline that is not itself an adversarial run is refused (`adversarial_baseline_run_not_adversarial`).

### Gate-3 mapping + locked precedence

`AdversarialGateInput` (`packs/approval_gates.py:314`) gains three fields — `regressions` / `regression_evaluated` / `candidate_run_id` (with defaults so every pre-13c construction stays valid; the default `candidate_run_id=None` reproduces the pre-13c `evidence_pointer=None`). `AdversarialRedReason` (`approval_gates.py:191`) gains a 4th value `adversarial_baseline_regression` (the consolidated `ApprovalGateRedReason` union auto-advances to 23 via `| AdversarialRedReason`). The composer threads `evidence_pointer=adversarial_input.candidate_run_id` (`approval_gates.py:478`, mirrors `signature_digest` on gate-1). The route reader `_build_adversarial_gate_input` (`portal/api/packs/review_routes.py:222`) maps `payload["adversarial"]` → `AdversarialGateInput` with **locked precedence**: `high_severity_failures > 0` → `regression_evaluated and regressions > 0` → `pass_rate < floor` → green. It is **fail-closed on a self-inconsistent or pointer-less snapshot** (a missing/non-string `candidate_run_id`, or regression sub-fields that contradict each other, route to `adversarial_evidence_not_attached`). The pre-13c reader's tolerant 2-key `{pass_rate, high_severity_failures}` shape was a never-populated forward-compat stub; the 6-key shape supersedes it (no persisted 2-key snapshots exist).

### Threshold + ISO + gate

- **Threshold stays operator-configured + Human-only.** The pass-rate floor is the existing Wave-1 `Settings.adversarial_pass_rate_floor` (tighten-only `ge`, default 0.99), threaded to `_build_adversarial_gate_input`'s `pass_rate_floor` param. 13c adds **no new Settings** and **no Alembic migration** — the snapshot is additive payload.
- **ISO:** no new control mapping — the submit chain row already tags its conformance/lifecycle controls; the adversarial snapshot rides the existing `pack.lifecycle.submitted` payload.
- **Critical-controls gate:** `evaluation/adversarial/evidence.py` promoted (count **124 → 125**, 95% line / 90% branch; 100/100 at promotion). `evaluation/storage.py` (`load_adversarial_verdict`), `packs/storage.py` (`payload_adversarial`), `packs/approval_gates.py` (the input/composer fields), `portal/api/packs/review_routes.py` (the reader), and `portal/api/packs/author_routes.py` (the producer wiring) all ride their **existing** gate entries; `portal/api/packs/dto.py` stays off-gate per the R32 precedent.

### Out of scope (recorded, not built)

- **The ADR-013 model-promotion adversarial gate** (`models/storage.adversarial_pass_rate`, live gate deferred) is a **separate** surface; it may reuse 13c's mechanism later but is not in 13c.
- **Auto-run at submit** — 13c references an existing 13b run; it does not run an adversarial suite at approval/submit time (no OS-only pack-as-target).

Sprint 13c closes the Sprint-13 arc (13a live replay → 13b adversarial testing → 13c promotion gate): the 5-gate composer is the promotion gate, gate-3 is now live-populated at submit from a referenced adversarial run with a baseline-regression sub-term, and the threshold stays a Human-only operator setting.
