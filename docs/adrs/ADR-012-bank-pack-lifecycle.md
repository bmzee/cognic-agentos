...
## M4 amendment — runtime-config provisioning + `disabled → installed` re-enable (2026-06-30)

Milestone **M4** (operator-grade pack install flow) makes the lifecycle state govern runtime callability. See **ADR-026** for the architectural decision (lifecycle-governed runtime configuration; desired/derived state; the materializer as the sole writer of the MCP carve-outs; `mcp_authz` unchanged; no live code-load). Two changes touch this ADR's lifecycle (the table at §23–48):

1. **New transition `disabled → installed` (re-enable).** The transition table lists `allow_listed → installed` (first install) but no re-enable path. M4 adds `disabled → installed` so a `disable`d pack can be re-activated without a new approval/allow-list cycle (its approval + allow-list state persist; the materializer re-projects the retained runtime-config record). `revoke` remains terminal (no re-enable). RBAC scope unchanged (`pack.install`).
2. **New `configure` step + runtime-config record.** Between `allow_listed` and `install`, the operator writes a per-`(tenant, pack)` runtime-config record (the MCP server-url override, internal-host allow-list entries, OAuth/AS references) via a new endpoint under a new **`pack.configure`** scope (human-actor-gated). `install` then materializes that record's derived MCP carve-outs and additionally verifies the pack is boot-registered/cosign-trusted, the config is complete, and the Vault references resolve — refusing on any miss. `disable`/`revoke` retract the derived carve-outs (the pack goes refused via the unchanged `mcp_authz` path). **Reconfiguring while `active` is refused** (a live config change is `disable → configure → install`) to avoid desired/derived drift absent a reconcile loop.

No change to the §41 five-gate approval composition, the approval override scope (`pack.override.approval_gate`), or any pre-existing transition **other than widening `install` as described above**. This is a **critical-controls** change to the lifecycle path; M4 lands it under `core-controls-engineer` / `/critical-module-mode` per AGENTS.md, with the new `pack.configure` scope + a new runtime-config store/table (Alembic migration).
