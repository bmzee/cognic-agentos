...
Enforcement — the carve-out applies at ALL THREE sandbox-create verification sites (drift correction, 2026-07-04): (1) `sandbox/admission.py` step 8 skips `catalog.verify_sbom_policy_or_refuse(...)` when `runtime_image_in_canonical_set` is true; (2) the DockerSibling egress-proxy sidecar verification (`sandbox/backends/docker_sibling.py::_start_proxy_sidecar`) and (3) the KubernetesPod proxy preflight (`sandbox/backends/kubernetes_pod.py::create`, step 4) do NOT call `verify_sbom_policy_or_refuse` on the proxy image — the proxy is REQUIRED to be canonical at both sites (non-canonical refuses outright at the membership gate with `sandbox_image_digest_not_in_canonical_catalog`), and canonical-image license acceptance is the release/signing decision above. Cosign verification still runs for canonical images at all three sites. This carve-out is narrow — it does NOT loosen the license posture for tenant/pack images (the "cannot loosen" rule above stands for that class; if the sidecar membership gate is ever relaxed to admit tenant-allow-listed proxies, the license gate MUST be re-introduced for that non-canonical class). *Drift note:* this amendment originally named only the admission site; the two sidecar sites predated it (Sprint 8A T10c R1 P1.1) and were missed by the Sprint 10.6 T30 implementation, so they kept running the license-deny policy on an image class the same code requires to be canonical. Surfaced by the M6 governed-skill proof (run 13, 2026-07-04) — the **first live execution** of the syft license gate: the canonical Debian-based egress proxy refused with 666 license-policy violations, exactly the failure this amendment's rationale predicted. Corrected at both sidecar sites then; cross-backend lockstep + real-catalog GPL/unlabeled regressions pinned at `tests/unit/sandbox/backends/test_proxy_sidecar_license_carveout.py`.
