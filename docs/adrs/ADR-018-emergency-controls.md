...
| **Sprint 13.6b (quotas) — LANDED** | The token quota meter: `core/emergency/quotas.py` `QuotaEngine` (decomposed atomic Redis counters — NOT Lua); `gateway_call_ledger` nullable token/cost evidence columns + migration `0010`; gateway tenant-aggregate quota gate + actual-usage metering; scheduler `QuotaInterrogator` conformance; read-only quota portal + `QuotaRBACScope`. See the Sprint-13.6b amendment below. The ADR-018 emergency arc CLOSES here (kill switches 13.6a + quotas 13.6b). |
...
Sprint 13.6a builds the **full ADR-018 kill-switch matrix** (the `memory.write_freeze` seed + the 7 ADR-table classes) with portal, RBAC, UI, gateway, and memory enforcement. **Quotas are deliberately SPLIT to Sprint 13.6b** — at the half-1 VALVE checkpoint the quota half (a `gateway_call_ledger` migration + a new CC-gated metering `core/emergency/quotas.py` + gateway/portal/RBAC re-touch) was carved into its own sprint with a fresh design checkpoint, rather than blurring 13.6 into a 12-task PR. **As of 13.6a the ADR-018 quota arc was NOT yet done** — it landed at Sprint 13.6b (the amendment below), which closes the ADR-018 emergency arc. (Note: the 13.6a checkpoint anticipated a Lua-metering module; the 13.6b design checkpoint REJECTED Lua for decomposed atomic counters — see the 13.6b amendment.)
...
- **Brake-before-evidence doctrine** — `flip`/`revert` write Redis FIRST (the brake takes effect even if evidence fails), THEN append the value-free chain rows `emergency.kill_switch_flipped` / `emergency.kill_switch_reverted` (ISO A.6.2.5 + A.9.2). An idempotent state-write skip is permitted only after a successful, well-formed authoritative Redis read proves the requested state is already stored; an authoritative absent key means effectively inactive. A GET failure, malformed document, or any local cache value never authorizes the skip, so SET is attempted and a SET failure precludes success evidence. Valid authoritative observations refresh the local cache before a possibly-failing SET, while malformed observations poison it active even when the repair SET fails; a later control-plane outage therefore cannot serve a cache value contradicted by the last authoritative observation. An evidence-append failure leaves the switch LIVE and surfaces the closed-enum `kill_switch_live_evidence_degraded` (the portal returns 502 `{switch_live: true}`; the idempotent re-flip converges the chain on retry). This SUPERSEDES the ADR's `PackKilled`/`OperationAborted` cross-surface exception names — each surface owns its own closed-enum taxonomy.
...
## Sprint 13.6b amendment (2026-06-13) — the token quota meter landed; the ADR-018 emergency arc is CLOSED

Sprint 13.6b builds the ADR-018 **token quota meter** — the second half of ADR-018. **With it the emergency arc CLOSES** (kill switches 13.6a + quotas 13.6b).

**Atomicity — "atomic counter reservation," NOT a single transaction (decomposed counters, NOT Lua).** The design checkpoint found the as-built Redis adapter exposes only `get`/`set`/`scan_iter` — the spec-sketched "single Lua eval" was not buildable without an adapter change. The landed design uses **decomposed atomic counters** via a consumer-owned `_AsyncRedisQuotaLike` Protocol (`incrby`/`decrby`/`get`/`set`/`getdel`/`expire`; the real async cache client satisfies it — NO shared `db/adapters` change): `would_admit` does `INCRBY`-first (the cumulative return makes each caller's admit decision concurrency-safe) → check `actuals + reserved ≤ limit` → rollback `DECRBY` on over-limit; `release_reservation` is `GETDEL`-first (atomic exactly-once → idempotent). There remains a **bounded crash / partial-increment window** between the counter mutation and the reservation-record persistence/release — this is ACCEPTED because every leak is **quota-DENYING, never quota-OPENING**, and self-heals at the 48h key TTL. The claim is never transactional all-or-nothing. A real-Redis concurrency proof is env-gated (`COGNIC_RUN_REDIS_INTEGRATION=1`, the Vault-Z2 precedent).

**Two planes (precision lock 2 — normative).** The **scheduler** has `task_id` + `pack_id` + `requested_estimated_tokens` → TRUE per-task reservation (`would_admit` at submit / `release_reservation` on every terminal — already wired since Sprint 10.5, now backed by the real `QuotaEngine` per the `QuotaInterrogator` seam). The **gateway** has neither pack identity nor a pre-dispatch completion-token count → a coarse tenant-aggregate **already-exhausted** gate (`check_gateway_admit`, at the F4 slot AFTER the kill-switch gate, BEFORE the rate-slot acquire) + a post-completion **actuals** `INCRBY` (`record_actuals`). The ADR does NOT imply exact pre-dispatch hard caps for gateway calls with unknown completion tokens; gateway quotas are tenant-aggregate, actual-usage-based.

**Duplicate-`task_id` contract.** Same-shape re-reservation is idempotent (no second increment — retried submits never double-charge); a mismatched-shape reuse fails loud (`QuotaReservationConflict` — a contract violation, never a quota outcome).

**Accumulation premise refined.** The ADR's §"Quotas" "accumulate via the gateway-call ledger ... eventually-consistent within 5s" is refined: the **Redis counter plane is the enforcement read** (the actuals counter the gateway increments + the scheduler reservations); the `gateway_call_ledger` nullable `prompt_tokens` / `completion_tokens` / `estimated_cost_usd` columns (migration `0010`, additive) are the **durable examiner evidence** (ADR-007 honesty preserved). `estimated_cost_usd` is evidence-ready (NULL — no pricing source).

**Limits + thresholds.** Kernel `Settings` defaults (`quota_tokens_per_tenant_per_day` / `quota_tokens_per_pack_per_day`) + per-tenant tighten-only ceilings via the ADR-023 overlay registry. Hard 100% → refuse (`quota_exhausted` at the gateway + a `quota.exhausted` chain row; `refused_quota_exhausted` at the scheduler, already wire-stable). Soft 80% → structured-log warn only (Wave-1).

**Closed-enum surfaces.** `GatewayTraceOutcome` 12 → 13 (+`quota_exhausted`); ledger `_ALLOWED_OUTCOMES` +`quota_exhausted`; typed `GatewayQuotaExhausted` (gateway) + `QuotaReservationConflict` (engine); NEW `QuotaRBACScope` (`quota.read` only); `core/emergency/quotas.py` promoted to the CC durable gate (128 → 129).

**The `PackKilled`/`OperationAborted` cross-surface exception names** (ADR §"Kill switches" prose) were already superseded at 13.6a by per-surface closed-enum taxonomies; 13.6b adds the gateway `quota_exhausted` + scheduler `refused_quota_exhausted` in the same per-surface spirit.

**Still deferred (NOT closed by 13.6b):** the operator override (`quota.override.tokens` + `quota.overridden` chain row + `PUT /api/v1/quotas`) — lands with the limit-write/override surface; spend-class enforcement — gated on a pricing source (the `estimated_cost_usd` column is evidence-ready now); tool-rate + subagent-depth quota classes — their consumers are unwired; the portal-wide `tenant_full` middleware; the Sprint-14 dashboard + Rego quota templates + per-tenant tunings; the **production scheduler DI binding** of the `QuotaEngine` (the composition-root sprint — 13.6b proves conformance by test). The gateway quota gate + the memory/gateway kill-switch enforcement ARE production-wired via `build_runtime`.
