...
| **Sprint 11.5b** | regulator-core: lifecycle (`forget`/`redact`/`regulator_erasure` + tombstone reaper), **real Redis `memory.write_freeze`** (replaces the 11.5a seam), scratch-Postgres fallback, core RBAC |
| **Sprint 11.5c** | surfaces: `export` (ADR-016 Sigstore reuse), `learning_surface` validator, portal `/memory` API, UI `memory.*` emit-hooks, surface RBAC, vector→real-recall wiring |
...
## Sprint 13.5c3 amendment (2026-06-12) — memory approval seam cutover (`core/memory/gate.py` Step 7; FIRST production-wired consult)

Sprint 13.5c3 makes `MemoryGate.check_write` the FOURTH approval-engine consumer seam — and the FIRST production-wired one: the `harness/runtime.py` MemoryAPI factory passes the unconditionally-built `runtime.approval_engine` (`runtime.py:225`), so deployments with high-tier contexts writing `long_term` get the ADR-014 pending → portal-grant → re-write flow instead of the hard transitional refusal. Behaviour change named honestly: strictly more functional; nothing weakened (the wired arm consults the engine; everything else is byte-identical).

1. **Two-arm cutover + kept fallback + the asymmetry preserved verbatim.** Step 7 (`gate.py:364`) keeps the EXACT §"Sprint 11.5 → Sprint 13.5 transitional rule" predicate — `tier == "long_term" AND ctx.risk_tier in _APPROVAL_REQUIRED_RISK_TIERS` (the 5-value set at `gate.py:117`; `customer_data_read` deliberately excluded per §"Integration with ADR-014 approval"'s "risk_tier >= customer_data_write") — on BOTH arms. Engine-absent raises `memory_approval_engine_not_available` byte-for-byte; recalls and `scratch`/`task` writes are NEVER consulted; the consult deliberately does NOT honour `tools.rego` overlay tightening below the 5-set (a bank gating lower tiers' long_term writes uses the tenant-overridable `long_term.allow` Rego point at Step 2). Steps 0–6 ALWAYS run first and re-run on every attempt — approval never bypasses them (zero approval rows on any Steps-0–6 refusal; pinned for DLP, consent, and kill-switch).
2. **SUPERSEDED sentences.** The §"transitional rule" claims that the refusal "is removed by Sprint 13.5" and that "the cutover emits a `memory_approval.engine_enabled` audit event" are EXPLICITLY SUPERSEDED, as is the phase table's "lifting the 11.5a transitional refusal" row: the refusal CONVERTS to the engine-absent arm (never removed), and the one-shot cutover event (the same process-once promise superseded at 13.5c1 + 13.5c2) is replaced by per-decision evidence — every `memory.write` row now carries `approval_verified` (always; `storage.py:272`) plus, on verified grants, the `approval_request_id` examiner join and (regulator tier) the `approval_audit_record_ref` forward edge.
3. **Binding contract.** `tool_identity = "memory:"+sha256(canonical_bytes({agent_id, operation}))` (`gate.py:131`); `args_digest` (`gate.py:142`) binds the approved write SHAPE INCLUDING CONTENT — tier / purpose / sorted data_classes / key|block_kind / subject / actor_id / risk_tier / `_value_digest(value)` (the SAME single-definition helper behind the chain row's `redacted_value_digest`, canonical home `core/memory/_digest.py` per the Layer-C storage fence). The engine never sees the raw value. Consent + retention are deliberately UNBOUND — Steps 0–6 re-validate them on every attempt (a consent regression at re-write surfaces as the consent reason). The regulator-tier `required_refs["audit_record_ref"]` is the HOISTED per-attempt `memory-write-*` id (`gate.py:359`) — an attempt-correlator whose forward edge is closed by the granted row's `approval_audit_record_ref`, read back trustworthily from the new `ApprovalCheckResult.required_refs` surface (never caller input).
4. **Surface deltas.** `MemoryRefusalReason` 18 → 23 (+5 `memory_approval_*`); `MemoryOperationRefused` gains the additive pending-only `approval_request_id` attr; `MemoryWriteRecord` +3 gate-set evidence fields; `remember()`/`upsert_block()`/`check_write()` gain the keyword-only `approval_request_id` carrier (malformed → `ValueError`, the caller-contract-bug class — the refusal vocabulary stays reserved for governance outcomes). `memory.rego` UNTOUCHED (no transitional refusal ever lived in the memory bundles). Cross-surface e2e at `tests/integration/approval/test_memory_seam_e2e.py`.

## M8 amendment (2026-07-05) — the ADR-027 agent loop writes task-tier digest-only run metadata; long-term agent memory is M9

ADR-027 (governed agent loop, M8) is the first LLM-agent consumer of the governed MemoryAPI: the kernel loop records what an agent run did — interpreted-question digest, chosen skill ids, scope ids, terminal state — as **task-tier, digest-only** writes under the agent's identity (`agent_id` = the acting agent; `actor_id` = the originating user — the ADR-027 dual identity). **The M8 boundary is structural, not conventional:** the loop constructs its `MemoryCallerContext` with `long_term_writes_allowed=False`, so a `long_term` write from the agent path is refused by this ADR's existing per-write gate regardless of pack declaration or tenant policy. **Long-term agent memory (remember / recall / forget / redact for agents) is M9**; the Wave-1.5 prompt-injection-of-memory hardening deferral stands unchanged. Additive; no gate, tier, or vocabulary change in this ADR.
...
**Memory events ride `decision_history`** (payload-only linkage — **no `core/audit.py` schema change**, no `core/canonical.py` / `decision_history.py` schema bump). The write and read events carry **different** payloads. **Write-side** events — `memory.write` (+ 11.5b `memory.forget` / `memory.redact` / `memory.regulator_erasure`) — are `DecisionRecord`s appended via `DecisionHistoryStore.append_with_precondition` (atomic with the `memory_records` row mutation), stamped ISO 42001 `A.7.4` / `A.8.2` / `A.8.5` / `A.10.2` on `iso_controls`; `memory.write` carries a **redacted-value-digest** (never the value) in the payload. **`memory.read`** is a plain `DecisionHistoryStore.append` (a read produces no value) carrying read-shaped metadata only (keyed `{tier, purpose, subject_ref, hit, record_id}`; enumerate `{op, …, subject_ref, hit, count}`) — **no value, no digest** — stamped ISO `A.7.4` / `A.8.2`.

**Cut-A split (a/b/c).** Ships as **11.5a** (governed substrate: 7-op surface, tiers/blocks, both enforcement gates, DLP seed, storage, default-deny Rego, erasure-ready schema, kill-switch *seam*) + **11.5b** (regulator-core: lifecycle `forget`/`redact`/`regulator_erasure` + tombstone reaper, real Redis `memory.write_freeze`, scratch-Postgres fallback, core RBAC) + **11.5c** (surfaces: `export` via ADR-016 Sigstore reuse, `learning_surface` validator, portal/UI, surface RBAC, vector→real-recall wiring). The original single 11.5b was split at the resolved 11.5a→11.5b valve (2026-06-01). The substrate is DI-tested, not harness-injected — same "primitive done, production wiring deferred" posture as the Sprint-11 sub-agent primitive.

## M8.5-C T1 amendment (2026-07-11) — HP-4: `memory_approval_originator_mismatch`

Per the ADR-014 M8.5-C T1 amendment (actor-bound grant replay, engine-owned): the memory gate's approval consult (`core/memory/gate.py`) threads `expected_originator_subject=ctx.actor_id` into `verify_grant_for_action` and maps the engine's `approval_originator_mismatch` to the NEW wire-public `MemoryRefusalReason` value **`memory_approval_originator_mismatch`** (the vocabulary grows 23 → 24). Actor identity remains inside the binding digest (the 13.5c2 refinement, defence-in-depth); the engine's originator check fires FIRST, so a swapped actor surfaces the uniform originator reason. Value-free: the refusal carries the bounded reason only.
