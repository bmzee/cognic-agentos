...
**Still deferred (14A-A2+):** backend-level checkpoint→wake (+ its approval correlator), scheduler-driven suspend/resume, sub-agent dispatch + the real `LocalParentBudgetResolver` + a top-level run→budget snapshot, multi-backend (K8s production), MCP `call_tool` exercise (a run route alone does not invoke MCP unless the workload calls a tool). *(Superseded 2026-06-19: the scheduler-backed parent-budget resolver landed as `SchedulerTaskParentBudgetResolver` (NOT a 14A-A2 `LocalParentBudgetResolver`); MCP `call_tool` landed via the MCP tool-invocation portal route; checkpoint→wake was wired at A3b/A3c. Still forward: live `SubAgentSpawner` sub-agent dispatch + the two-budget-design reconciliation + a top-level run→budget snapshot.)*
...
## Sprint 14A-A3c amendment (2026-06-16) — the wake-approval correlator: run-record wake-pending lifecycle + the no-re-mint guard (`core/run/executor.py` `resume()`; `POST /api/v1/runs/{run_id}/resume`)

Sprint 14A-A3c is the third and final WIRING slice of the checkpoint→wake / run-persistence arc (A3a substrate → A3b resolver/resume → **A3c wake approval correlator**). It crosses the A3b "A3c fence": the wake-path sandbox `admit_policy` is now threaded with the approval engine (ADR-004 owns that sandbox angle), and the executor's `resume()` gains the run-record wake-pending lifecycle so a high-risk wake can pend → grant → re-resume exactly like cold-create. The synchronous `POST /api/v1/runs` lane and the A3b resume route shape stay STABLE — A3c only extends `resume()` + the resume DTO. **No new on-gate module** — the edits land in modules already on the gate (`core/run/executor.py`, `sandbox/protocol.py`, both backends); `core/run/_types.py` + the dto/routes stay off-gate. CC count stays **131**.

1. **The run-record wake-pending lifecycle (matrix +4 pairs; the 9-value vocabulary UNCHANGED).** `resume()` gains keyword-only `approval_request_id: uuid.UUID | None`; the resumable-state guard widens from `suspended` to `record.state ∈ {suspended, pending_approval}`, and `from_state = record.state` (the three A3b hardcoded `"suspended"` transitions are converted to read the live state). When `backend.wake()` raises `sandbox_approval_pending`, the executor walks the run record `suspended → pending_approval` on the **first** resume (persisting the minted `approval_request_id` on the run row) and emits the existing `run.pending_approval` output row + the store-side `run.lifecycle.pending_approval` lifecycle row (the `suspended→pending_approval` transition; the store keys the decision-type by the target state); the route returns **202** carrying the `approval_request_id`. `core/run/_types.py` (off-gate) folds `_A3C_VALID_TRANSITIONS` into the 3-way `_VALID_TRANSITIONS` union (+4 producible pairs: `suspended→pending_approval`, `pending_approval→{woken,refused,failed}`). The **9-value `RunState` vocabulary is UNCHANGED**, honoring the A3a "expand-matrix-only, never the vocabulary" locked doctrine across all three slices.

2. **The no-re-mint guard (the F2 design pin).** A run already in `pending_approval` MUST supply its stored `approval_request_id` on the re-resume: a missing id raises `RunResumePendingApprovalRequired` (**409 `run_resume_approval_id_required`**) and a mismatched id raises `RunResumeApprovalMismatch` (**409 `run_resume_approval_id_mismatch`**) — BOTH raised BEFORE `backend.wake()` is dispatched, so a `pending_approval` run can NEVER re-enter sandbox admission Arm A (mint-a-fresh-request). This makes the silent new-pending loop unrepresentable: a re-resume either carries the stored id (Arm B = verify-the-grant) or is refused at the executor before any wake. The still-pending arm (a re-resume that pends AGAIN under a real engine) emits NO transition — `pending_approval → pending_approval` is not a legal pair, so a self-loop row is never written (it would be misleading evidence).

3. **resume still makes NO scheduler calls; claim-gated teardown is unchanged.** A3c does not change the A3b resume-side contracts: `resume()` re-submits nothing, re-admits through no `scheduler.rego`, and takes no quota reservation (`task_id` is always None on the resume `RunResult`) — quota/scheduler-on-resume remains the deliberate forward item. The pending arm and the refused arm both RETURN before the atomic `suspended → woken` claim, so they own nothing and the resume `finally` never destroys the session; the claim-gated teardown (the wake/tombstone race fix) and the orphaned-backend-resource forward item are exactly as A3b left them.

**On-gate surface (CC).** `core/run/executor.py` (the `resume()` pending arm + the no-re-mint guard + the widened resumable-state guard), `sandbox/protocol.py` + both backends (the wake-path approval threading + the approval-family wrapper exemption — see the ADR-004 Sprint-14A-A3c amendment). `core/run/_types.py` (off-gate) expanded the matrix with `_A3C_VALID_TRANSITIONS` (+4 pairs) — vocabulary UNCHANGED. The resume DTO/route (`RunResumeRequest.approval_request_id` + the 2 new 409 mappings; the `pending_approval → 202` + `_run_response_from_result` retry contract is automatic — both pre-existed at A3a/A3b) + the e2e are off-gate. CC count stays **131** (no new gate module; `admission.py` / `checkpoint_store.py` / `storage.py` are consumed, not edited).

**Live-proof posture (honest scope — high-risk live exercise DEFERRED, the F4 deferral).** The wake-approval seam is WIRED + unit-proven (+ an env-gated stub-engine real-docker e2e at `tests/integration/run/test_managed_run_resume_approval_e2e.py`, `COGNIC_RUN_DOCKER_SANDBOX=1`). But the production run route submits the minimal **`read_only`** run shape, which auto-tiers under a real `ApprovalEngine` and **never pends** — so the wake-approval seam is wired-but-not-live-exercised-in-production for `read_only`, exactly the 14A-A2 cold-create posture. The env-gated e2e drives a STUB/conformer approval engine specifically because the `read_only` shape auto-tiers under a real engine, forcing the pending→grant→re-resume cycle the production path cannot yet produce. The manifest-driven high-risk run shape that would live-exercise both the cold-create and the wake approval seams in production is a separate later slice.

**A3c completes the checkpoint→wake arc's WIRING.** The resolver + resume route landed in A3b; the wake approval correlation lands in A3c. The remaining items (the manifest-driven high-risk run shape, orphaned-backend-resource reconciliation, quota/scheduler-on-resume, the real `LocalParentBudgetResolver` + a top-level run→budget snapshot, live sub-agent dispatch, resumption UX) are independent forward tracks, NOT arc dependencies. *(Superseded 2026-06-19: the parent-budget resolver landed as `SchedulerTaskParentBudgetResolver`; the high-risk run shape landed at 14A-A4b. Still forward: live `SubAgentSpawner` sub-agent dispatch + the two-budget-design reconciliation + a top-level run→budget snapshot.)* See the ADR-004 Sprint-14A-A3c amendment for the sandbox wake-threading / wrapper-exemption / no-`CheckpointMetadata`-change angle, and the ADR-014 Sprint-14A-A3c amendment for the 5th `admit_policy` approval consumer + the no-re-mint guard.

## Sprint 14A-A4a amendment (2026-06-16) — scheduler `approval-delegated-to-sandbox` admission affordance (`core/scheduler/{engine,policy,storage}.py` + `scheduler.rego`)

Sprint 14A-A4a adds an explicit, audited way for the scheduler to admit a **high-risk** task *because the downstream sandbox admission gate owns the human checkpoint* — without minting its own approval and without faking `approval_verified`. It ships **additive and dormant**: nothing in production sets the signal yet (the A4b managed-run executor is the only future setter). This section + the ADR-014 Sprint-14A-A4a amendment together ARE the coordinated kernel + ADR loosening of `scheduler.rego` that the Sprint-10.5 bank-overlay contract requires for loosening kernel defaults (the bundle change itself landed in this sprint). **CC count stays 131** — the edits land in modules already on the gate (`engine.py` / `policy.py` / `storage.py`) + the stop-rule `scheduler.rego` + the off-gate `_types.py`; no new gate module, no migration.

1. **The named delegate signal (off-gate `_types.py`).** A new closed enum `SchedulerApprovalDelegate = Literal["sandbox_admission"]` + a defaulted `SubmitInput.approval_delegated_to: SchedulerApprovalDelegate | None = None`. It is a **routing/evidence** signal, NOT a grant carrier or verification attestation — so it is deliberately **excluded from the scheduler approval binding digest** `_submit_args_digest` (the 6-key actor-bound digest is unchanged; the disposition-map drift pin at `test_approval_seam.py` grows a `routing_or_evidence` bucket + a behavioural exclusion pin proving the digest is invariant under the field).

2. **Engine boundary validation + skip-consult (CC `engine.py`).** `submit()` validates the field at the boundary (the `approval_request_id` mirror): an unknown value, or co-presence with a *syntactically valid* `approval_request_id`, raises `SchedulerSubmitInputInvalid(field="approval_delegated_to")`. **Precedence-pinned** — the existing unconditional `approval_request_id` UUID parse runs FIRST, so a malformed id surfaces `field="approval_request_id"` and mutual-exclusion fires only on the parsed valid id. `SchedulerSubmitInputInvalidField` grows 2 → 3. Step-3.5 then **skips the approval consult entirely when delegated** (`if approval_delegated_to is not None: pass` ahead of the `elif approval_engine is not None:` consult): no `create_request`, no mint, and `approval_verified` stays **False** (honest — the scheduler verified nothing). Non-delegated submits keep the 13.5c2 consult byte-identical.

3. **The Rego loosening — allow arm 3 (CC `policy.py` + stop-rule `scheduler.rego`).** `policy._build_rego_input` threads a 10th key `approval_delegated_to` (always, nullable; `null` fails closed over the OPA boundary). `scheduler.rego` gains a THIRD allow arm: a high-risk tier admits on strict `input.approval_delegated_to == "sandbox_admission"`. With A4a the bundle has **3 allow arms** — arm 1 (safe tiers), arm 2 (13.5c2: high-risk on `approval_verified == true`), arm 3 (A4a: high-risk on delegation) — and a **10-key** input. The high-risk **refusal** arm gains a third conjunct (`not input.approval_delegated_to == "sandbox_admission"`) so it never mislabels a delegated-admitted input; the **3-value refusal vocabulary is UNCHANGED** (no new value, no wire rename). Delegation does not bypass the other admission gates — pack-state / kill-switch / quota / caps still apply (the rego is one gate in the pipeline).

4. **Honest accepted-row evidence (CC `storage.py`).** The `scheduler.admission_accepted` chain row carries `approval_delegated_to` **only when non-None**, alongside the always-present `approval_verified=False` and **no** scheduler `approval_request_id` (the grant-correlator key is gated on `approval_verified=True`, so a fake grant is structurally unrepresentable on the delegated path). An examiner reads "high-risk, admitted because approval is delegated to the sandbox admission gate," not a forged scheduler grant.

5. **The normative setter obligation (spec §3.8) + dormancy.** A4a cannot verify that a delegating caller actually routes the work through sandbox admission — the scheduler has no handle on the downstream gate — so the affordance is governed by a **normative contract on the setter, audited per-admission on the chain row**: any caller setting `approval_delegated_to="sandbox_admission"` MUST route the same unit of work through sandbox admission carrying the real manifest risk tier. **Sprint 14A-A4b is the only authorized production setter** (the managed-run executor, which constructs the sandbox `PackAdmissionContext` with the same real tier). Until A4b, no production caller sets the signal → the new rego arm is rego/unit-tested but never live on `main`, and `read_only` evidence stays honestly `read_only`. Misuse (delegating without a downstream gate) would be a scheduler high-risk bypass — flagged in the `scheduler.rego` arm-3 header.

## Sprint 14A-A4b amendment (2026-06-17) — A4a goes LIVE: the managed-run executor is the production setter (`core/run/executor.py`)

Sprint 14A-A4b is the **activator** for the A4a affordance above. It makes the managed-run executor source a pack's *validated manifest risk tier* and set `approval_delegated_to="sandbox_admission"` on the scheduler submit — so A4a's `scheduler.rego` arm 3, rego/unit-tested-but-dormant since A4a, is now **live-exercised on a production code path**. **CC count stays 131** (the edits land in `core/run/executor.py`, already on the gate since 14A-A; the manifest loader is off-gate in `harness/sandbox.py`); **no migration** — the tier is read off the existing `pack.lifecycle.submitted` chain row, not a new column.

1. **The executor discharges the A4a §3.8 setter obligation by construction.** `ManagedRunExecutor.run()` resolves the pack's manifest tier (the off-gate `PackRecordStoreLoader` reads `payload["manifest"]["risk_tier"]["tier"]` off `find_latest_submit_row(load_lifecycle_history(...))`), validates it fail-closed, and threads the SAME validated tier into BOTH the scheduler `SubmitInput` (`pack_risk_tier=record.risk_tier` + `data_classes` + `approval_delegated_to="sandbox_admission"`) AND the sandbox `PackAdmissionContext`. The delegating caller therefore routes the same unit of work through sandbox admission with the real tier — no high-risk bypass.

2. **Fail-closed tier (I1) — never a `read_only` downgrade.** `_validate_pack_record` gains two checks (tier first, after `installed`): an unresolved/non-canonical `risk_tier` refuses `pack_record_risk_tier_unresolved`; a present-but-malformed `data_classes` refuses `pack_record_data_classes_malformed` (`RunRefusalReason` 4 → 6). A run whose manifest tier cannot be trusted is REFUSED, never silently admitted as `read_only` — the dishonest-evidence failure mode Architecture-Z exists to prevent.

3. **Honest scheduler evidence is now emitted by a live caller.** A high-risk managed run produces a `scheduler.admission_accepted` row carrying the REAL tier (e.g. `customer_data_read`) + `approval_delegated_to="sandbox_admission"` + `approval_verified=False` — the A4a shape, now real. `read_only` runs still admit via arm 1 (the flag is inert for safe tiers).

4. **Posture (honest).** Live-exercised at the unit level (the `@opa_required` `test_a4b_delegated_high_risk_admitted_by_real_rego_arm3` drives the executor's delegated-high-risk submit through the REAL `scheduler.rego`) + end-to-end via the env-gated real-docker e2e `tests/integration/run/test_managed_run_high_risk_e2e.py` (`COGNIC_RUN_DOCKER_SANDBOX=1`) proving cold-create high-risk pend → grant → re-POST → complete under a REAL `ApprovalEngine`. The production `POST /api/v1/runs` route now flows a pack's real manifest tier to both gates; a high-risk pack pends at the sandbox cold-create checkpoint, a `read_only` pack still never pends (honest).

## Sprint 14A-A4c amendment (2026-06-17) — the high-risk WAKE run-lifecycle live-exercised; F4 fully closed

Sprint 14A-A4c closes the "F4" caveat on the run-lifecycle surface: the high-risk **suspend → resume / WAKE** path is now live-exercised under a REAL `ApprovalEngine`. A4b live-exercised the high-risk cold-create; A4c walks the durable run through its full high-risk lifecycle — `pending → running → suspended → pending_approval → woken → completed` — across two approval cycles. **Recon-proven e2e-only — zero production-code change** (A3b/A3c wired the suspend/resume/wake-pending lifecycle + the no-re-mint guard; A4b threads the high-risk tier into the `PackAdmissionContext` persisted at suspend). **CC 131; no migration; no new on-gate module.**

1. **The two-cycle, two-`run_id` shape.** Cycle 1 (cold-create, A4b): `run(suspend_after_exec=True)` pends (that run is abandoned at `pending_approval`) → grant → re-POST (a fresh `run_id`) admits via Arm B → exec → `session.suspend()` → `suspended`. Cycle 2 (wake, A3c): `resume(run_id)` re-pends at wake → grant → re-resume → `woken` → exec → `completed`. `resume` makes NO scheduler calls (the slot was freed at suspend; `task_id` stays None). The transition matrix already covers every pair.
2. **Live-proof.** The env-gated real-docker e2e `tests/integration/run/test_managed_run_high_risk_wake_e2e.py` asserts the durable run's full 6-state lifecycle walk + the `run.suspended` / `run.pending_approval` / `run.completed` evidence rows. This supersedes the A3c amendment's "manifest-driven high-risk run shape" forward item — with A4b (cold-create) + A4c (wake), the high-risk managed-runtime governance story has zero asterisks. The remaining forward tracks (orphaned-backend-resource reconciliation, quota/scheduler-on-resume, MCP `call_tool`, the real `LocalParentBudgetResolver` + sub-agent dispatch, resumption UX) are independent of the approval-seam work. *(Superseded 2026-06-19: MCP `call_tool` landed via the MCP tool-invocation portal route; the parent-budget resolver landed as `SchedulerTaskParentBudgetResolver`. Still forward: live `SubAgentSpawner` sub-agent dispatch + the two-budget-design reconciliation.)*

## Parent budget resolver seam amendment (2026-06-19) — Fork-E resolved: the real resolver wired at `build_runtime`

The Sprint-13.7 "Parent-budget = `_Null` sentinel (deferred to 14A)" note (§308 — the OMITTED `parent_budget_resolver`) and the seam-table default (§259 — "`_NullParentBudgetResolver` raises `NotImplementedError`") are now **resolved**. `build_runtime` constructs the real **`core/scheduler/budget_resolver.py::SchedulerTaskParentBudgetResolver`** (reusing the scheduler's `SchedulerStorage` as the snapshot `reader`) and injects it as `parent_budget_resolver=`, replacing the sentinel.

- **The resolver** reads the parent task's granted `requested_estimated_tokens` via the new pure-read `SchedulerStorage.get_budget_snapshot(task_id, *, tenant_id)` (the `WHERE task_id AND tenant_id` is the cross-tenant boundary), and applies the policy: absent/cross-tenant → `ParentTaskBudgetUnavailable("parent_not_found")`, terminal → `ParentTaskBudgetUnavailable("parent_terminal")`, else the granted ceiling. The engine's existing `compute_child_budget` narrows the child to `min(child_quota, parent_granted)`.
- **The seam Protocol gained `*, tenant_id`** (the engine's `submit()` call-site passes `submit_input.tenant_id`); the dormant Sprint-11b `subagent/` consumers got a minimal signature-only compat touch (`conformers.py::LocalParentBudgetResolver` ignores it; `spawn.py::_resolve_budget` threads `request.tenant_id`).
- **The composition e2e flipped** — `tests/integration/scheduler/test_scheduler_composition_e2e.py::test_composition_subagent_submit_fails_loud` now asserts a sub-agent submit with a random `parent_task_id` raises **`ParentTaskBudgetUnavailable("parent_not_found")`** (the real resolver reads it as absent), NOT the `_NullParentBudgetResolver`'s `NotImplementedError`. (The §308 note + the §259 seam-table default describe the pre-this-slice state; the test name is unchanged so the `::name` xrefs stay valid.)
- **CC 131 → 132** (`budget_resolver.py` on the durable per-file gate + registered in the no-emergency/no-sandbox architecture-guard exhaustiveness lists); `get_budget_snapshot` on the already-on-gate `storage.py`; `_seams.py` + `harness/runtime.py` off-gate. No migration; no `SchedulerRefusalReason` / `scheduler.rego` change. **Deferred to the next slice:** the live sub-agent dispatch caller (wiring `SubAgentSpawner`), the two-budget-design reconciliation (`compute_child_budget` vs `compute_spawn_budget`), the sibling/shared-pool ledger.
