...
## Proof 1b-1 packaging remediation (2026-06-24)

Proof 1b-1 (the deployed-loop proof — a kind/Helm instance must boot the kernel and trust-register a real signed pack) drove the deploy path live on kind for the first time and surfaced that **the deployed kernel could not boot at all** — a set of packaging drifts between this ADR's stated contract and the actual Dockerfile/chart. This remediation closes the three packaging defects; the broader migration-hook-ordering defect (Gap 3) is explicitly **deferred** to a reviewed follow-up. **Proof 1b-1 is NOT claimed green by this change** — it unblocks the kernel boot + migration config so the deployed registration claim (and the kind/AKS `/readyz` smoke confidence) can be re-validated against a kernel that actually boots.

### Implementation drift fixed (Gap 7 + Gap 5 — image packaging)
- **`policies/` was never packaged.** This ADR (above) states the image "ships the OPA policy bundles in-image under `/app/policies/_default/`", but `infra/agentos/Dockerfile` copied only `src` / `pyproject.toml` / `README.md` — **not** the repo-root `policies/`. So `build_runtime`'s `OPAEngine.create(bundle_path=settings.tools_policy_bundle)` (the default resolves to `policies/_default/tools.rego` relative to `/app`) raised `RegoBundleNotFoundError` and the pod `CrashLoopBackOff`ed before serving traffic. **Fixed:** both runtime stages (`runtime` = `create_app`; `default-adapters` = `create_prod_app`) now `COPY --chown=root:cognic policies ./policies`.
- **`alembic.ini` was never packaged.** This ADR states deployed migrations run `alembic upgrade head` "from the same image (it bundles … the migrations dir)", but the image omitted `alembic.ini` (the `script_location` config), so `alembic` found no config. **Fixed:** both runtime stages now `COPY --chown=root:cognic alembic.ini ./`.
- Both stages add `RUN chmod -R a+rX /app/policies /app/alembic.ini` so the non-root `cognic` user can read them regardless of the build-context umask (a robustness guarantee; on a clean checkout the build context is already `0644`). CMD/entrypoint unchanged. The kernel (`runtime`) stage receives the bundles for completeness + `/app` layout parity; it remains without the `cosign`/`opa` binaries per the existing kernel-image posture, so a full OPA-gated boot there still requires an operator to add `opa`.

### Migration env parity fixed (Gap 6 — chart)
The values-gated migration **hook Job** set only `COGNIC_DATABASE_URL`, so `env.py`'s `get_settings()` ran under the prod profile with prod-only settings (e.g. `embedding_model`) falling back to dev defaults — and the strict-profile guard refused `get_settings()` **before** `alembic upgrade head` ran. **Fixed:** `templates/migration-job.yaml` now `envFrom`s the chart ConfigMap (`{{ include "agentos.fullname" . }}-config`) — the same non-secret `COGNIC_*` config the Deployment inherits. The byte-snapshots (all 8 scenarios) regenerate with exactly this `envFrom` block.

### Deferred (NOT fixed here): migration hook ordering (Gap 3)
The migration Job is a Helm **pre-install/pre-upgrade hook** that references **three** normal (non-hook) resources Helm creates **after** pre-install hooks: the ServiceAccount, the bootstrap Secret, and — as of the Gap 6 fix above — the `{{ include "agentos.fullname" . }}-config` ConfigMap. So a fresh `helm install` deadlocks (`serviceaccount … not found`; the Secret + ConfigMap refs fail the same way). Fixing this is a real change to the "operator owns migrations" hook posture (above) and must be weighed across both the chart-created-Secret and ESO (`Z1b-b`) secret modes (e.g. a post-install hook with a crash-tolerant Deployment, vs. SA/Secret/ConfigMap-as-earlier-weight hooks). **Any migration-sequencing follow-up MUST preserve the Gap 6 ConfigMap env parity** — the migration must continue to `envFrom` the same non-secret `COGNIC_*` config the Deployment sees, or the strict-profile `get_settings()` refusal returns. It is **out of scope** for this packaging remediation and tracked as a follow-up; the Proof 1b-1 harness sidesteps it with a non-hook migration Job run post-install, when all three normal resources exist.
