...
| **Skill** | A `SKILL.md` package (`cognic-skill-*`) — the agentskills.io-standard folder: YAML frontmatter `name` + `description`, a Markdown instructions body, optional `references/` and `assets/`. **A skill may optionally carry one governed action.** | yes |
