...
- The packs exist to demonstrate the **per-kind Wave-1 author lifecycle**: tool packs run `scaffold → validate → harness (PASS) → sign → verify`; skill + agent packs run `scaffold → validate → harness (refused with closed-enum `harness_unsupported_pack_kind` per T13/R31's `_HARNESS_SUPPORTED_KINDS = frozenset({"tool"})` narrow) → sign → verify`. Sign + verify are kind-agnostic and DO NOT narrow — all three packs ship full attestations + manifests. (R10 P3 #3 reviewer correction — earlier draft said "scaffold → validate green-path"; R6 P2 #3 promoted T15 to the full-lifecycle gate; R32 P2 #3 amended T15 Step 3 with the per-kind expectation matrix; R33 P2 #2 lines THIS Doctrine D paragraph + the test inventory at line 1386 + the closeout criteria at line 1585 up with that same matrix so a future T15 author cannot reintroduce uniform PASS-for-all-three expectations.) NOT to ship Layer C agent behavior. Neutral names (`example-minimal` not `policyqa`) prevent reviewers / banks from mistaking them for production templates with domain-specific assumptions.
...
- Modify: `.gitignore` (R10 P2 #1) — add the **explicit exception lines** for the agent example pack's keypair. Tool + skill example packs do NOT need a signing keypair (no AgentCard JWS to sign), so only the agent pack gets the exception lines. The exact lines T15 commits:
  ...
  ```
- Create: `tests/unit/cli/test_reference_packs_full_lifecycle_green.py` (R6 P2 #3 — renamed from `test_reference_packs_validate_green.py`; the reference packs gate the **per-kind author lifecycle** matrix per R33 P2 #2: tool pack runs `scaffold → validate → harness (PASS) → sign → verify`; skill + agent packs run `scaffold → validate → harness (refused with `harness_unsupported_pack_kind` per T13/R31) → sign → verify`. Filename retains "full_lifecycle" because sign + verify cover all three packs end-to-end; only the harness step is per-kind. The test MUST encode the matrix explicitly: `assert tool_report.overall_status == "pass"` AND `assert any(f.reason == "harness_unsupported_pack_kind" for f in skill_report.findings)` AND same for `agent_report` — uniform PASS-for-all-three expectations would contradict T13/R31's narrowing of `_HARNESS_SUPPORTED_KINDS`).
...
- ✅ All 4 BUILD_PLAN exit criteria (line 550-554) are pinned by tests in T15 (reference packs **per-kind lifecycle green** per R6 P2 #3 + R33 P2 #2 — scaffold-fixture-on-disk + validate + harness (PASS for tool, `harness_unsupported_pack_kind` refusal for skill + agent per T13/R31's kind narrow) + sign + verify, NOT just validate; R7 P3 #5 refresh — earlier wording said "validate-green" which would have understated the closeout evidence; R33 P2 #2 refresh — earlier wording said "harness" applied uniformly to all three packs, which contradicted T13/R31's narrowing of `_HARNESS_SUPPORTED_KINDS = frozenset({"tool"})`) + T6 (orchestrator) + T13 (test-harness produces conformance report).
