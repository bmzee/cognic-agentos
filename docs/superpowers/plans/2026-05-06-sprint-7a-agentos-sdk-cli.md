...
3. **Dry-run dispatch** for tool packs (R33 P2 #1 narrow): the harness loads each `cognic.tools` entry-point declared in the pack's `pyproject.toml` via filepath (`importlib.util.spec_from_file_location` against `pack_path/src/`), instantiates the class with NO constructor arguments (`cls()`), and invokes `await tool.invoke()` with NO kwargs against the SDK's already-validated `Tool.invoke` template (which validates input + output schemas). Skill + Agent packs are explicitly refused by the kind-narrowing gate per T13/R31 (closed-enum reason `harness_unsupported_pack_kind`) — their dispatch contracts (Skill `ToolRegistry` at instantiation, Agent `handle(payload: bytes, *, task: TaskRecord)`) require dedicated fixture-adapter wiring that lands in a future harness-expansion task.
4. **Wave-1 narrow contract — no transport interception** (R33 P2 #1 reviewer correction): the harness DOES NOT install `httpx.MockTransport`, inject `agentos_sdk.testing.fixture_settings`, scope environment variables, or sandbox filesystem / network access. Pack `_invoke()` code runs against the unmodified host runtime — real httpx / hvac / sqlalchemy / Langfuse clients hit their live targets if the pack constructs them. Pack authors who need fixture-adapter isolation wire it themselves via `agentos_sdk.testing.fixture_settings` / `fixture_audit_capture` in their pack test suite (the harness is a pre-publish sanity gate — validate pipeline + Tool.invoke dispatch + conformance report — NOT a sandbox). Earlier drafts of this bullet promised aspirational transport sandboxing via a `harness_unsupported_live_transport` closed-enum refusal that was never implemented; the closed-enum `HarnessReason` literal in `cli/test_harness.py` has always reflected the narrow (no transport-interception reason exists). R33 P2 #1 narrowed the docstrings + this plan paragraph + Doctrine D reference-pack lifecycle prose (line 74) + T15 test inventory (line 1386) + closeout criteria (line 1585) in one pass to match the implementation. The Wave-1 narrow is pinned as documentation-as-code by `test_cli_test_harness.py::test_run_harness_wave1_narrow_contract_no_transport_interception`. Transport interception is a future expansion alongside the kind-dispatch-table widening.
...
**Why this Wave-1 narrow over mock-everything or real-AgentOS** (R34 P2 #2 — earlier draft titled this section "Why hybrid over mock-everything or real-AgentOS" and described a fixture-adapter middle ground that was never built):
- Mock-everything would let a pack pass harness checks while failing on first registry admission; the harness's load-bearing role is surfacing the same `validate` refusals the runtime trust gate would, plus a `Tool.invoke` schema-validation dry-run.
- Real-AgentOS would require a live Vault, a live Postgres, a live MCP server — too heavy for `pip install agentos-cli && agentos test-harness`. The Wave-1 narrow runs the validate pipeline + a tool-pack-only `cls() + await instance.invoke()` dry-run against the unmodified host runtime; pack `_invoke()` code that constructs real httpx / hvac / sqlalchemy / Langfuse clients hits their live targets (no transport interception). Pack authors who need fixture-adapter isolation use `agentos_sdk.testing.fixture_settings` / `fixture_audit_capture` directly in their pack pytest suite — the harness is a pre-publish sanity gate, not a sandbox.
...
- The packs exist to demonstrate the **per-kind Wave-1 author lifecycle**: tool packs run `scaffold → build wheel → sign --bundle → validate → harness (PASS) → verify`; skill + agent packs run `scaffold → build wheel → sign --bundle → validate → harness (refused with closed-enum `harness_unsupported_pack_kind` per T13/R31's `_HARNESS_SUPPORTED_KINDS = frozenset({"tool"})` narrow) → verify`. Sign + verify are kind-agnostic and DO NOT narrow. **Static-only-committed-state doctrine (T15 R-round 1)**: committed packs ship manifest + pyproject + inert source + README + agent-card.json seed + (agent only) test-only keypair under `attestations/test-signing/`. Generated attestations (the seven supply-chain files + agent-card.jws) are produced under `tmp_path` during the lifecycle test and never committed in `examples/` — eliminates the drift risk that pre-generated committed attestations would carry, and the test exercises the realistic author flow where `sign --bundle` populates `supply_chain.attestation_paths` BEFORE `validate` checks they exist. (R10 P3 #3 reviewer correction — earlier draft said "scaffold → validate green-path"; R6 P2 #3 promoted T15 to the full-lifecycle gate; R32 P2 #3 amended T15 Step 3 with the per-kind expectation matrix; R33 P2 #2 lines THIS Doctrine D paragraph + the test inventory at line 1386 + the closeout criteria at line 1585 up with that same matrix so a future T15 author cannot reintroduce uniform PASS-for-all-three expectations; Sprint-7A T17-followup R-round corrected the lifecycle order from `validate → sign` to the canonical `sign → validate` here + at line 1386 + lines 1493-1498 + line 1585 to match the static-only-committed-state doctrine actually shipped at T15.) NOT to ship Layer C agent behavior. Neutral names (`example-minimal` not `policyqa`) prevent reviewers / banks from mistaking them for production templates with domain-specific assumptions.
...
src/cognic_agentos/cli/test_harness.py                 — Doctrine C runner (validate pipeline + tool-pack-only `Tool.invoke` dispatch dry-run; R33/R34 narrow — no fixture-adapter injection / no transport interception)
...
tests/unit/cli/test_reference_packs_full_lifecycle_green.py  — R6 P2 #3 + R32/R33/R34 — per-kind matrix: tool pack runs scaffold + validate + harness (PASS) + sign + verify; skill + agent packs run scaffold + validate + harness (refused with `harness_unsupported_pack_kind` per T13/R31) + sign + verify; sign + verify are kind-agnostic and DO NOT narrow
```
...
Per Doctrine Decision C (post R31 + R32 + R33 narrowing — see lines 41-57 for the canonical version that all task bodies must match), `agentos test-harness <pack-path>`:
1. Run the full validate pipeline (every refusal surfaces).
2. Pack-kind narrowing gate (R31 P2 #2 + R34 P2 #2): for `kind="tool"` packs, proceed to step 3. For `kind="skill"` or `kind="agent"` packs, emit closed-enum refusal `harness_unsupported_pack_kind` and skip dispatch — Skill + Agent dispatch contracts (Skill `ToolRegistry` at instantiation, Agent `handle(payload: bytes, *, task: TaskRecord)`) require dedicated fixture-adapter wiring that lands when the harness is expanded.
3. For tool packs: load each `cognic.tools` entry-point declared in the pack's `pyproject.toml` via filepath (`importlib.util.spec_from_file_location` against `pack_path/src/`), instantiate the class with NO constructor arguments (`cls()`), invoke `await tool.invoke()` with NO kwargs against the SDK's already-validated `Tool.invoke` template (which validates input + output schemas), and capture the response_keys for the conformance report.
4. **Wave-1 narrow contract — no transport interception** (R33 P2 #1 / R34 P2 #2): the harness DOES NOT install `httpx.MockTransport`, inject `agentos_sdk.testing.fixture_settings`, scope environment variables, or sandbox filesystem / network access. Pack `_invoke()` code runs against the unmodified host runtime. Earlier drafts of this task body said "instantiate with `agentos_sdk.testing.fixture_settings()`" + "`httpx.MockTransport` everywhere; SecretAdapter returns fixture keyring; AuditStore is in-memory; LangfuseAdapter is no-op" — that wiring was never implemented; the closed-enum `HarnessReason` literal in `cli/test_harness.py` has always reflected the narrow (no `harness_unsupported_live_transport`-style reason exists). Pack authors who need fixture-adapter isolation use `agentos_sdk.testing.fixture_settings` / `fixture_audit_capture` in their pack test suite (the harness is a pre-publish sanity gate, not a sandbox). Transport interception is a future expansion alongside the kind-dispatch-table widening; the narrow is pinned as documentation-as-code by `test_cli_test_harness.py::test_run_harness_wave1_narrow_contract_no_transport_interception`.
...
- Modify: `.gitignore` (R10 P2 #1) — add the **explicit exception lines** for the agent example pack's keypair. Tool + skill example packs do NOT need a signing keypair (no AgentCard JWS to sign), so only the agent pack gets the exception lines. The exact lines T15 commits:
  ...
  ```
- Create: `tests/unit/cli/test_reference_packs_full_lifecycle_green.py` (R6 P2 #3 — renamed from `test_reference_packs_validate_green.py`; the reference packs gate the **per-kind author lifecycle** matrix per R33 P2 #2: tool pack runs `scaffold → build wheel → sign --bundle → validate → harness (PASS) → verify`; skill + agent packs run `scaffold → build wheel → sign --bundle → validate → harness (refused with `harness_unsupported_pack_kind` per T13/R31) → verify`. Filename retains "full_lifecycle" because sign + verify cover all three packs end-to-end; only the harness step is per-kind. The test MUST encode the matrix explicitly: `assert tool_report.overall_status == "pass"` AND `assert any(f.reason == "harness_unsupported_pack_kind" for f in skill_report.findings)` AND same for `agent_report` — uniform PASS-for-all-three expectations would contradict T13/R31's narrowing of `_HARNESS_SUPPORTED_KINDS`. Sprint-7A T17-followup R-round corrected the lifecycle order from `validate → sign` to the canonical `sign → validate` to match the static-only-committed-state doctrine: committed reference packs do NOT ship pre-generated attestations, so `agentos validate`'s `supply_chain.attestation_paths` non-empty + all-present check refuses on a clean checkout until `sign --bundle` populates the attestation set.).
...
- [ ] **Step 1: Generate the three packs as STATIC-ONLY committed artifacts** — manifest + entry-point pyproject + minimal source + README + agent-card.json seed (agent only) + test-only RSA-2048 keypair under `attestations/test-signing/` + NOTE.md (agent only). Generated attestation files (the seven supply-chain files + agent-card.jws) are NOT committed; they are produced under `tmp_path` during the lifecycle test (Sprint-7A T17-followup R-round correction; aligns the plan with the static-only-committed-state doctrine T15 actually shipped).
- [ ] **Step 2: Stage clone in tmp_path + build the wheel + run `agentos sign --bundle`** — copies the static-only committed pack into `tmp_path`, synthesizes a Wave-1 wheel into `<pack>/dist/` from the committed `pyproject.toml` + `src/` source, then runs `agentos sign --bundle` which populates `attestations/` with the seven files + (agent only) regenerates `agent_cards/agent-card.jws`. Sign uses the committed test-only signing keypair under `attestations/test-signing/` for the agent-pack JWS.
- [ ] **Step 3: Run `agentos validate` against the now-populated tmp_path tree → expect PASS for all three** (zero refusals; warnings allowed since they exit 0 by design). Sign-before-validate matters: the supply-chain validator's `attestation_paths` non-empty + all-present check refuses on a clean checkout because committed packs ship no attestations; sign produces them, then validate clears.
- [ ] **Step 4: Run `agentos test-harness` against each → expect PASS for the tool pack; expect `harness_unsupported_pack_kind` refusal (exit 1 + stderr `::error` annotation) for the skill + agent packs** (R32 P2 #3 reviewer correction — earlier draft said "PASS for all three" which conflicted with T13/R31's explicit narrowing of the harness's Wave-1 dispatch table to `kind="tool"` only. Skill + Agent dispatch contracts (Skill needs `ToolRegistry` at instantiation; Agent's `handle()` takes `payload: bytes + task: TaskRecord`) require dedicated fixture-adapter wiring that lands in a follow-up harness-expansion task — Sprint-7B or a dedicated Sprint-7A retroactive task — when `cli/test_harness.py:_HARNESS_SUPPORTED_KINDS` grows from `frozenset({"tool"})` to `frozenset({"tool", "skill", "agent"})` AND the dispatch path routes per-kind via a kind-aware `_dispatch_one`. T15's `test_reference_packs_full_lifecycle_green.py` MUST encode the per-kind expectation matrix: `assert tool_report.overall_status == "pass"` AND `assert any(f.reason == "harness_unsupported_pack_kind" for f in skill_report.findings)` AND same for agent_report. Sign + verify do not narrow.).
- [ ] **Step 5: Run `agentos verify` against each (using the freshly signed bundle from Step 2)** — expect exit 0 + all 7 verifier reasons absent (R7 P2 #2 — was 6). For the agent pack, verify uses the committed public-key PEM as the trust root for AgentCard JWS verification.
- [ ] **Step 6: Add `tests/unit/cli/test_reference_packs_full_lifecycle_green.py`** to make the full lifecycle (stage-clone + build wheel + sign + validate + harness + verify) a CI gate against the committed `examples/` tree. If any committed static artifact drifts (manifest schema break, missing template, broken pyproject), the gate fires.
- [ ] **Step 7: Commit.**
...
- ✅ All 4 BUILD_PLAN exit criteria (line 550-554) are pinned by tests in T15 (reference packs **per-kind lifecycle green** per R6 P2 #3 + R33 P2 #2 + Sprint-7A T17-followup R-round — stage-clone + build wheel + sign --bundle + validate + harness (PASS for tool, `harness_unsupported_pack_kind` refusal for skill + agent per T13/R31's kind narrow) + verify, NOT just validate; sign-before-validate per the static-only-committed-state doctrine since committed reference packs ship no attestations and `validate`'s `supply_chain.attestation_paths` non-empty + all-present check requires `sign --bundle` to have populated the attestation set first; R7 P3 #5 refresh — earlier wording said "validate-green" which would have understated the closeout evidence; R33 P2 #2 refresh — earlier wording said "harness" applied uniformly to all three packs, which contradicted T13/R31's narrowing of `_HARNESS_SUPPORTED_KINDS = frozenset({"tool"})`; Sprint-7A T17-followup R-round refresh — earlier wording listed `validate → harness → sign → verify` which contradicted the static-only-committed-state doctrine T15 actually shipped) + T6 (orchestrator) + T13 (test-harness produces conformance report).
