...
| `portal/rbac/tenant_isolation.py` | **NEW per Round 1 P2 #3** — route-level tenant guard. `RequireTenantOwnership(pack_id)` dependency that loads `PackRecord.tenant_id` and verifies actor.tenant_id match. Mismatch returns 404 (not 403, to avoid information leak). Closed-enum 3-value `TenantIsolationFailure` literal (`tenant_id_mismatch` / `pack_not_found` / `actor_tenant_id_missing`). |
...
- `src/cognic_agentos/portal/rbac/tenant_isolation.py` — **NEW per Round 1 P2 #3.** `RequireTenantOwnership(pack_id_param: str)` FastAPI dependency factory; loads the `PackRecord` via `PackRecordStore.load(pack_id)`; verifies `pack.tenant_id == actor.tenant_id`; returns the loaded `PackRecord` on success. Closed-enum 3-value `TenantIsolationFailure` literal: `tenant_id_mismatch` (returns 404 to avoid info leak per OWASP "verbose error messages" guidance) / `pack_not_found` (also 404) / `actor_tenant_id_missing` (returns 500 — kernel misconfig; an actor with no tenant_id should never have been bound). `PackRecord.tenant_id` exists today at `packs/storage.py:289` per Sprint-7B.1 T3.
...
- Create: `src/cognic_agentos/portal/rbac/tenant_isolation.py` per Round 1 P2 #3 — `RequireTenantOwnership(pack_id_param)` dependency factory; loads `PackRecord` via `PackRecordStore.load(pack_id)` from `app.state.pack_record_store`; verifies `pack.tenant_id == actor.tenant_id`; returns the loaded `PackRecord` on success. Closed-enum 3-value `TenantIsolationFailure` literal: `tenant_id_mismatch` (404) / `pack_not_found` (404) / `actor_tenant_id_missing` (500 — kernel misconfig).
...
Produce halt summary with: files modified + tests run + tests passed + closed-enum vocabulary established + reviewer-watchpoint-to-pinning-regression map for: (a) scope literal stability (12 values), (b) ActorType literal stability (2 values), (c) Actor model frozen + actor_type field, (d) ActorBinder Protocol contract, (e) RequireScope dependency closed-enum denial reasons (3 values), (f) TenantIsolationFailure closed-enum (3 values; 404 not 403), (g) RequireHumanActor closed-enum (1 value; admits human refuses service), (h) **structured-HTTP-only denial scope** (Round 2 P2 #4 — NO chain-event emission in 7B.2). Wait for `commit` token.
...
- Modify: `tools/check_critical_coverage.py` — bump floor 43 → 54 + add 11 new module entries with rationale lines:
  ...
  4. `portal/rbac/tenant_isolation.py` — Round 1 P2 #3 — cross-tenant 404 + TenantIsolationFailure closed-enum
...
(User may flag additional conflicts after Round 10 patches; I patch into this file before T1 plan-commit.)
