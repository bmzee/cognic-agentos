...
- Modify: `src/cognic_agentos/sandbox/backends/kubernetes_pod.py` (same shape as Docker — Session field already shipped at T10 Docker as a Protocol-compat shim; T10 K8s replaces the `NotImplementedError` shim with the real mint loop + warm-pool short-circuit + 3-arm cleanup envelope including `asyncio.CancelledError` + destroy revoke loop + checkpoint/suspend Q5 lock helper. Imports `_mint_exception_to_refusal_reason` from `src/cognic_agentos/sandbox/backends/_shared_credentials.py` — NO duplicate mapping table. **Note**: the helper was originally written into `docker_sibling.py` at T10 Docker; T10 K8s round-2 reviewer-P1 (Gap I) caught the optional-extra coupling that an import from `docker_sibling` would create and promoted the helper out to the dependency-neutral `_shared_credentials` module before the K8s commit landed.)
- Create: `tests/unit/sandbox/backends/test_kubernetes_pod_credentials.py` (NEW pure-unit file per Gap F carry-forward — the env-gated `test_kubernetes_pod_lifecycle.py` stays untouched; mirrors `test_docker_sibling_credentials.py`'s 7-batch structure minus the 3 invariants covered by the cross-backend file, plus K8s-specific regressions: pre-flight proxy refusal does NOT mint)
- Modify: `tests/unit/sandbox/test_credential_lifecycle.py` (extend the cross-backend conformance `backend_kind` parametrize to include `"k8s"` + add the K8s fixture builders; the 3 invariants — mint mapping × 4 params, destroy fail-soft, Q5 lock × 2 — then run for BOTH backends so cross-backend drift is the load-bearing regression)
...
git add src/cognic_agentos/sandbox/backends/kubernetes_pod.py \
        tests/unit/sandbox/backends/test_kubernetes_pod_credentials.py \
        tests/unit/sandbox/test_credential_lifecycle.py
...
*Round 3 (2026-05-24, T10 Docker code-time — 3 doctrinal gaps surfaced + closed in the commit; 1 Gap F naming carry-forward applied to the K8s fileset at T10 K8s pre-flight):*

- **Gap F** (surfaced at T10 Docker code-time, 2026-05-24) — plan T10 fileset said `Modify: test_docker_sibling_lifecycle.py` (and the K8s sibling `test_kubernetes_pod_lifecycle.py`). Both files are env-gated on `COGNIC_RUN_DOCKER_SANDBOX=1` / `COGNIC_RUN_K8S_SANDBOX=1` respectively. Pure-unit tests for the new T10 wiring belong in NEW non-env-gated files matching the existing `_audit_emission` / `_checkpoint_unit` / `_egress_proxy_seam` per-concern convention. Resolution at T10 Docker commit (`4841c93`): created `test_docker_sibling_credentials.py` instead; the env-gated lifecycle file stays untouched (Z2 real-Vault integration proof is the natural extension home). T10 K8s pre-flight carries the same fix forward — created `test_kubernetes_pod_credentials.py` instead of touching `test_kubernetes_pod_lifecycle.py`. Plan fileset entries at lines 80-81 + the T10 K8s code-commit fileset + the T10 K8s `git add` block all updated to match.

- **Gap G** (surfaced at T10 Docker round-2 reviewer P1, 2026-05-24) — initial T10 Docker create() had THREE nested try/except blocks (mint-loop catch, topology catch, lifecycle.created catch) where the inner catches swallowed only Vault taxonomy exceptions, leaving non-Vault audit failures (DH chain head conflicts, canonical-form rejections, arbitrary RuntimeError from emit) as lease-leak paths. Resolution at T10 Docker round-2: refactored to a SINGLE post-admission cleanup envelope with TWO except arms — Vault taxonomy + generic Exception — both converging on a shared `_cleanup_post_admission_failure` helper that revokes minted leases best-effort. Carry-forward to T10 K8s: same shape required (Vault + generic Exception arms + shared helper).

- **Gap H** (surfaced at T10 Docker round-3 reviewer P1, 2026-05-24) — Python 3.8+ `asyncio.CancelledError` subclasses `BaseException` NOT `Exception` (MRO: `[CancelledError, BaseException, object]`; `issubclass(asyncio.CancelledError, Exception) is False`). The Gap-G refactor's `except Exception` arm DID NOT catch cancellation, so a create() task cancelled mid-flight after mint would skip cleanup and leak the lease. Resolution at T10 Docker round-3: added explicit `except asyncio.CancelledError:` arm BEFORE the `except Exception:` arm — cleanup helper called, cancellation re-raised UNCHANGED (never swallow cancellation; Vault TTL is the final safety net for nested re-cancel during cleanup). Carry-forward to T10 K8s: SAME three-arm cleanup envelope shape — `asyncio.CancelledError` arm comes first, then Vault taxonomy, then generic Exception.

*Round 4 (2026-05-24, T10 K8s code-time — 2 reviewer P1 + P2 fixes; both cross-backend, the second one carries back to Docker since T10 Docker already shipped the same bug at commit `4841c93`):*

- **Gap I** (surfaced at T10 K8s round-2 reviewer P1, 2026-05-24) — the user-locked T10 K8s scope said "Reuse Docker's `_mint_exception_to_refusal_reason`; no duplicate mapping table." Round-1 K8s implementation imported the helper directly from `docker_sibling.py`. The reviewer verified with a blocked-import probe (`sys.modules['aiodocker'] = None`) that importing `cognic_agentos.sandbox.backends.kubernetes_pod` then fails with `ModuleNotFoundError: import of aiodocker halted` because `docker_sibling.py` imports `aiodocker` at module load. That broke the optional-extra boundary documented at `sandbox/__init__.py:99` where K8s-only deployments explicitly do NOT install the `sandbox-docker` extra. Resolution at T10 K8s round-2: created `src/cognic_agentos/sandbox/backends/_shared_credentials.py` (dependency-neutral; imports ONLY from `core.vault` exceptions + `sandbox.protocol.SandboxRefusalReason`). Moved `_mint_exception_to_refusal_reason` from `docker_sibling.py` to the new module; BOTH backends now import from the shared module. Mirrors the precedent set by `_shared_exec.py`. The K8s import comment carries the doctrine + the blocked-import-probe regression pattern.

- **Gap J** (surfaced at T10 K8s round-2 reviewer P2, 2026-05-24; carry-back to Docker required) — both `destroy()` bodies (Docker + K8s) awaited `_teardown_session_state(...)` BEFORE the active-lease revoke loop. K8s teardown intentionally propagates non-404 `ApiException` per its own docstring ("fail-closed so a real teardown failure does not silently leak"); Docker teardown swallows only `DockerError` so non-Docker exceptions (daemon unreachable, network OSError, etc.) propagate identically. If teardown raised, the function exited before the revoke loop and active leases sat in Vault until server-side TTL. Reviewer flagged on K8s; the same bug shape exists on Docker since T10 Docker commit `4841c93`. Resolution at T10 K8s round-2: wrapped teardown in `try/finally` on BOTH backends with the revoke loop in the `finally` branch (gated on `not already_destroyed` so idempotent second-destroy doesn't re-emit; audit emits wrapped in `contextlib.suppress(Exception)` so audit-DB failures during finally don't shadow the original teardown exception per Python's finally-block semantics). The teardown exception still propagates after the finally — semantically correct (the session WASN'T cleanly destroyed) — but examiners + SOC have audit evidence that the leases WERE revoked. Cross-backend invariant pinned by NEW `TestCrossBackendDestroyRevokesEvenWhenTeardownRaises` in the parametrized conformance file; runs for BOTH `["docker", "k8s"]` so any future destroy-flow regression on either backend fires at one site.

*Round 5 (2026-05-24, T10 K8s code-time — 3 reviewer findings on the round-2 fix's sharp edges; all cross-backend, all bundled into the same T10 K8s commit):*

- **Gap K** (surfaced at T10 K8s round-3 reviewer P1, 2026-05-24) — the Gap-J round-2 fix wrapped audit emits in `contextlib.suppress(Exception)` UNCONDITIONALLY to prevent the audit-emit exception from shadowing the teardown exception on the teardown-failure path. But that also silently suppressed audit-emit failures on the NORMAL path (teardown succeeded), violating spec §7.2's bank-grade contract: "banks have audit evidence for every revoke failure". Resolution: conditional-suppress per a `teardown_succeeded` flag (set `True` on the line immediately AFTER the successful teardown await). Per-iteration nested-async helper `_emit_revoke_event(coro)` wraps the emit in `suppress(Exception)` ONLY when `teardown_succeeded=False`. Normal path now propagates audit-emit failures so operators see audit-chain problems; teardown-failure path still preserves the original teardown exception per Python finally-block semantics. Cross-backend invariant pinned by NEW `TestCrossBackendDestroyAuditEmitConditionalSuppress` (3 tests × 2 backends = 6 runs).

- **Gap L** (surfaced at T10 K8s round-3 reviewer P2, 2026-05-24) — the Gap-I round-2 `_shared_credentials.py` docstring claimed the dependency-neutral contract was "pinned by the test-only AST scan at `tests/unit/sandbox/backends/test_shared_credentials.py`", but the test file didn't exist. Resolution: created the missing file with 3 regression classes — (1) AST scan asserting `_shared_credentials.py` imports ONLY `__future__` + `cognic_agentos.core.vault` + `cognic_agentos.sandbox.protocol`; (2) subprocess blocked-import probe spawning `python -c "..."` with `sys.modules['aiodocker'] = None` injected to assert `kubernetes_pod` imports clean (mirrors the reviewer's verification method; subprocess isolation prevents parent-process contamination); (3) per-Vault-exception mapping regression × 4 parametrize rows. The AST + subprocess combination catches both build-time (the import statement appears in source) and runtime (the coupling shows up at import time) regressions even if a future refactor introduces the coupling through a path the AST scan does not see.

- **Gap M** (surfaced at T10 K8s round-3 reviewer P3, 2026-05-24) — cross-backend conformance file's module docstring at `tests/unit/sandbox/test_credential_lifecycle.py:21` still pointed at `sandbox/backends/docker_sibling.py:_mint_exception_to_refusal_reason` even after the Gap-I move. Resolution: updated to `sandbox/backends/_shared_credentials.py:_mint_exception_to_refusal_reason` + added a reference to the new `test_shared_credentials.py` dependency-contract pin so future readers find the regression home.
