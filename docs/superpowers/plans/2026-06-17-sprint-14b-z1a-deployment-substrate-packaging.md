...
**Prerequisites for the executor (local):** the **primary** render/snapshot tool is **Helm v4.2.2** and the schema validator is **kubeconform v0.8.0** — both must be on PATH for T1–T5 (local `helm` v4.2.2 is the acceptable verification tool for T1–T4; the Homebrew Helm 4 already installed is the primary stack, so there is **no need to replace it with Helm 3 locally**). Helm-3 deployability — the chart is `apiVersion: v2` and stays installable by Helm 3 banks/OpenShift regardless of the validator — is proven by a **separate CI compatibility lane** (T5) that installs a pinned **Helm 3** binary (`helm3`, v3.16.3) and runs render + schema validation against it (no byte-snapshot diff on the Helm 3 render). `kind` + `docker` for the T6 smoke (env-gated; verified-by-reading is acceptable if the executor lacks a Docker host — flag it).
...
Mirrors `tests/unit/portal/api/ui/test_well_known_routes.py`: byte-equality vs a committed render; first run creates the snapshot and fails (telling you to commit it); regenerate via `rm <file> && uv run pytest …`. The snapshot is generated by the **primary Helm v4.2.2** renderer; pinned release name `rel` + namespace `cognic` + `ci/snapshot-values.yaml` make the render deterministic, and **local `helm` v4.2.2 is the acceptable verification tool for T1–T4**. The test skips (does not fail) if `helm` is unavailable, so CI without helm is honest rather than red. (The Helm 3 compatibility render is NOT byte-snapshotted — see T5; compatibility means a successful render + schema validation, not byte equality.)
...
```yaml
  helm-chart:
    name: helm4 lint/template/kubeconform/snapshot + helm3 compatibility
    runs-on: ubuntu-latest
    timeout-minutes: 8
    env:
      HELM_VERSION: "v4.2.2"        # primary renderer + snapshot generator (matches the local Homebrew Helm 4)
      HELM3_VERSION: "v3.16.3"      # compatibility-lane pin — the chart is apiVersion v2, so it stays Helm-3-deployable
      KUBECONFORM_VERSION: "v0.8.0"
    steps:
      - name: Checkout
        uses: actions/checkout@v6
      - name: Install primary Helm 4 (as `helm`, checksum-verified)
        run: |
          set -euo pipefail
          curl -fsSLo helm4.tgz "https://get.helm.sh/helm-${HELM_VERSION}-linux-amd64.tar.gz"
          curl -fsSLo helm4.sha256 "https://get.helm.sh/helm-${HELM_VERSION}-linux-amd64.tar.gz.sha256sum"
          echo "$(cut -d' ' -f1 helm4.sha256)  helm4.tgz" | sha256sum -c -
          mkdir -p helm4 && tar -xzf helm4.tgz -C helm4
          sudo install -m 0755 helm4/linux-amd64/helm /usr/local/bin/helm
          helm version --short
      - name: Install compatibility Helm 3 (as `helm3`, checksum-verified)
        run: |
          set -euo pipefail
      ...
      # --- Helm 3 compatibility gate: render + schema ONLY, NO snapshot diff ---
      - name: helm3 lint (compatibility)
        run: helm3 lint infra/charts/agentos -f infra/charts/agentos/ci/snapshot-values.yaml
      - name: helm3 template (compatibility)
        run: |
          helm3 template rel infra/charts/agentos --namespace cognic \
            -f infra/charts/agentos/ci/snapshot-values.yaml > /tmp/rendered-helm3.yaml
      - name: kubeconform schema validation (Helm 3 output)
        run: kubeconform -strict -summary -kubernetes-version 1.27.0 /tmp/rendered-helm3.yaml
```
...
```
Expected: workflow YAML parses; snapshot matches; kubeconform reports `0 errors` (hook-annotated Job + all objects valid). This local check uses the primary **Helm v4.2.2** + **kubeconform v0.8.0**; the **Helm 3 compatibility lane** runs in CI (which installs a pinned `helm3`). If the executor also has a local `helm3`, run `helm3 lint`/`helm3 template … > /tmp/rendered-helm3.yaml` + `kubeconform … /tmp/rendered-helm3.yaml` to confirm compatibility locally (no snapshot diff on the Helm 3 render).
...
- [ ] **Step 4: Always-on chart gates green locally + report versions**
...
if command -v helm3 >/dev/null 2>&1; then
  helm3 lint infra/charts/agentos -f infra/charts/agentos/ci/snapshot-values.yaml
  helm3 template rel infra/charts/agentos --namespace cognic -f infra/charts/agentos/ci/snapshot-values.yaml > /tmp/rendered-helm3.yaml
  kubeconform -strict -summary -kubernetes-version 1.27.0 /tmp/rendered-helm3.yaml
  echo "helm3 compatibility: PASS (local)"
else
  echo "helm3 not local — Helm 3 compatibility verified in the CI helm-chart job"
fi
```
Expected: primary (Helm 4) lint clean, snapshot matches, kubeconform 0 errors; the Helm 3 compatibility lane lints + renders + schema-validates with 0 errors (NO snapshot diff on the Helm 3 render — compatibility is successful render + schema, not byte equality). **The closeout report MUST state the primary Helm + kubeconform versions AND the Helm 3 compatibility check result (local or CI).**
...
- [ ] **Step 6: Final stop — no commit here** (T1–T7 already committed). Report the full-gate evidence — including the primary **Helm v4.2.2 + kubeconform v0.8.0** versions and the **Helm 3 compatibility** check result (chart `apiVersion: v2` proven Helm-3-deployable) — and hold for the PR/merge tokens.
