...
- [ ] **Step 3: Implement.**
`dto.py` — `SubAgentSpawnRequestBody` gains `approval_request_id: uuid.UUID | None = None`; `SubAgentSpawnResponse` gains `approval_request_id: str | None = None`. `ChildResultBody` gains `terminal_state: str | None = None` (so the pending state is visible in the body).
...
- [ ] **Step 4: Operator e2e (env-gated `COGNIC_RUN_DOCKER_SANDBOX=1`)** — new `tests/integration/run/test_subagent_approval_retry_e2e.py`: high-risk child spawn → `202` + `approval_request_id` → real `grant()` → re-POST with the id → `200` completed; assert the chain carries `subagent.return(pending_approval)` then a fresh `subagent.spawn → return(completed)`. Module-collectable-skips-without-docker (copy the `test_managed_run_high_risk_e2e.py` harness incl. the safe/high-risk `pack.lifecycle.submitted` manifest).
