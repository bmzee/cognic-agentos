...
- [ ] **Step 7: Run + facade + types** — `uv run pytest tests/unit/subagent/ -q && uv run ruff check src/cognic_agentos/subagent/ tests/unit/subagent/ && uv run mypy src tests`. The facade (`subagent/_facade.py`) delegates to `spawn()`; update its `spawn_subagent` signature + `tests/unit/subagent/test_subagent_facade.py` to drop `class_`/`pack_kind`/`pack_risk_tier` + the separate `pack_id`/`child_argv`, and add `managed_run: ManagedRunChildSpec` + `actor: Actor`. Expected: PASS; clean.
