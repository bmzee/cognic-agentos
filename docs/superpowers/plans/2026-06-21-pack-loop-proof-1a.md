...
def test_discover_finds_cognic_tool_search() -> None:
...
def test_real_sign_output_provisions_into_resolver_layout(tmp_path: Path) -> None:
    _require_toolchain()
    ...
    pack_obj = next(
        p for p in PluginRegistry().discover() if p.record.distribution_name == "cognic-tool-search"
    )
