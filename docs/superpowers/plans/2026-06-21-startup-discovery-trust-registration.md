...
- [ ] **Step 4: Docs.** ADR-002 amendment (startup discovery/trust-registration + `pack_attestation_root_path`) + ADR-016 cross-ref (first production caller of the full attestation pipeline) + ADR-003 cross-ref + AS_BUILT milestone + spec→LANDED + the both-surfaces closeout-language update. **(b):** with `mcp_admission` wired in the lifespan, MCP-served `[tool.cognic.mcp]` packs now register at boot too — both the MCP host AND the A2A receiver resolve against a real catalog; `mcp_admission=None` preserves the kernel-image doctrine (MCP packs refused when the deps are absent).
