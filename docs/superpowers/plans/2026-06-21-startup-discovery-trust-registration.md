...
# (b) expansion — the mcp_admission seam, over the REAL register gate (reuse the
# tests/unit/protocol/test_mcp_registration_auth_probe.py mocked-supply-chain + MCP-manifest pattern):
async def test_mcp_intent_pack_registers_when_mcp_admission_wired(...) -> None: ...  # [mcp] pack + a real/minimal MCPAdmissionDeps -> NOT refused (status "registered")
async def test_mcp_intent_pack_refused_when_mcp_admission_omitted(...) -> None: ...   # same pack, mcp_admission=None -> "mcp_admission_deps_required" (preserved doctrine path)
```
Run → FAIL.

- [ ] **Step 2: Implement `build_and_populate_registry`.** Signature `build_and_populate_registry(*, settings, audit_store, supply_chain, object_store, mcp_admission: MCPAdmissionDeps | None = None) -> PluginRegistry` — **NO `trust_gate` param; the boot owns the `registration_trust_gate`**. **AS-BUILT:** `decision_history_store` was DROPPED — no collaborator (`PluginRegistry` / `register_with_full_attestation_check` / `TrustGate` / `SupplyChainPipeline`) consumes one; the pipeline's evidence is the `audit_store` hash chain. **AS-BUILT (b) — the `mcp_admission` seam:** passed-in dependency wiring (like `supply_chain`/`object_store`), threaded into the register call so a `[tool.cognic.mcp]` pack clears the MCP admission gates instead of being refused fail-closed with `mcp_admission_deps_required`; default `None` preserves the kernel-image doctrine; the builder NEVER constructs the deps (the lifespan does — Sprint 4); it is NOT a trust gate (never touches the trapdoor). Read `root = settings.pack_attestation_root_path`; if `None` → log `pack_attestation_root_unconfigured` + return a fresh empty `PluginRegistry(audit_store=…)` (NO discovery loop). Else: `registration_settings = settings.model_copy(update={"signature_root_path": Path(root)})` → `registration_trust_gate = TrustGate(settings=registration_settings, ...)`; resolve `cosign_trust_root = Path(settings.trust_root_prefix) / "_default" / "cosign.pub"` (the LOCKED convention — **NOT `signing_trust_root_path`**) and **fail-close if it is missing / not a file / empty** (raise a boot-builder error → §5 both-503, distinct from the benign unset-root); it is under `trust_root_prefix` by construction (`verify_pack_signature` re-canonicalizes there as defence-in-depth); load the `_default` allow-list (fail-closed typed error on missing/malformed); fresh `PluginRegistry(audit_store=…)`; `for pack in registry.discover():` → `try: att = resolve_pack_attestations(pack, pack_attestation_root=Path(root), cosign_trust_root=cosign_trust_root); await registry.register_with_full_attestation_check(pack, att, trust_gate=registration_trust_gate, supply_chain=…, object_store=…, tenant_id="_default", tenant_allowlist=<frozenset>, mcp_admission=mcp_admission) except (PackAttestationResolutionError, Exception) as exc: log + skip`. Return the registry.
...
- [ ] **Step 2: Implement the unification.** Predeclare `app.state.plugin_registry = None` (body). In the SDK-capable lifespan path, BEFORE the MCP + A2A blocks: build `supply_chain = SupplyChainPipeline(settings=settings)` + `object_store` + **(b) the MCP admission deps** `mcp_admission = MCPAdmissionDeps(settings=settings, vault_client=adapters.secret, opa_engine=None, make_authz_client_for_probe=<fresh `MCPAuthzClient` factory for probe isolation>)` — **AS-BUILT:** `MCPAuthzClient`/`MCPAdmissionDeps` are **SDK-FREE** to construct, so `mcp_admission` is built **UNCONDITIONALLY** on the adapter path (MCP packs admittable on the kernel image; only the MCP *serving* host stays SDK-gated). **`opa_engine=None` (accepted)** keeps the boot off the OPA-binary path → a sampling-capable MCP pack **default-denies** (`mcp_sampling_default_denied`; also default-denied today by `tenant_sampling_permitted=False`), non-sampling MCP packs register; wiring a real `OPAEngine(bundle_path=settings.mcp_sampling_policy_bundle)` is a documented extension. **Sprint 5 closeout MUST NOT overclaim sampling-capable MCP registration.**; `registry = plugin_registry or await build_and_populate_registry(settings=settings, audit_store=…, supply_chain=supply_chain, object_store=object_store, mcp_admission=mcp_admission)` inside the fail-soft try (allow-list-failure exception → `registry = None` + ERROR). **AS-BUILT: no `decision_history_store` kwarg** (the boot dropped it — Sprint 3). **The boot builds its OWN `registration_trust_gate` internally — do NOT pass the `a2a_trust_gate` (the §4 trapdoor).** Set `app.state.plugin_registry = registry`. Replace `mcp_registry = plugin_registry or PluginRegistry(...)` (`:646`) + `a2a_registry = …` (`:689`) with the shared `registry`; if `registry is None`, skip both host/endpoint construction (→ both `None` → 503).
...
- [ ] **Step 4: Docs.** ADR-002 amendment (startup discovery/trust-registration + `pack_attestation_root_path`) + ADR-016 cross-ref (first production caller of the full attestation pipeline) + ADR-003 cross-ref + AS_BUILT milestone + spec→LANDED + the both-surfaces closeout-language update. **(b):** with `mcp_admission` wired in the lifespan, **non-sampling** MCP-served `[tool.cognic.mcp]` packs register at boot too — both the MCP host AND the A2A receiver resolve against a real catalog; **sampling-capable packs default-deny** (`opa_engine=None` accepted; also `tenant_sampling_permitted=False` today — the closeout MUST NOT overclaim sampling-capable MCP registration); `mcp_admission=None` would preserve the kernel-image doctrine (MCP packs refused when the deps are absent).
