...
- [ ] **Step 4: failing store tests** — `tests/unit/core/mcp_config/test_storage.py` (against the migrated DB; mirror `tests/unit/core/config_overlay/` if present, else the migrated-DB fixture):
  ...
  - (c) **override grammar:** `validate_override_url("http://10.42.0.7:8080/mcp")` accepted; `validate_override_url("https://10.42.0.7")` raises `override_url_not_http`; `validate_override_url("http://svc.ns.svc.cluster.local")` raises `override_url_host_not_ip_literal`; `validate_override_url("ftp://x")` raises `override_url_not_http`; `validate_override_url("not a url")` raises `override_url_malformed`; `validate_override_url(123)` raises `override_url_not_string`.
