...
- **The AS issuer** = `http://192.88.99.9:9000` — used across: the MCP server's `COGNIC_PROOF_AS_ISSUER`, the Vault path segment `as_host` = `192.88.99.9_9000`, and the AS's `_AS_ISSUER` (→ `token_endpoint` + `issuer`). **Trailing-slash caveat (attempt-4 finding):** the MCP server wraps `COGNIC_PROOF_AS_ISSUER` in pydantic `AnyHttpUrl`, which normalises it to `http://192.88.99.9:9000/`, so its PRM advertises `authorization_servers: ["http://192.88.99.9:9000/"]` and the Vault `mcp-as-allowlist` `servers[0]` MUST seed that slash-suffixed form (`seed-vault.sh` writes `${AS}/`) — the kernel compares the issuer by EXACT string per RFC 8414 (`mcp_authz.py:753`). `192.88.99.9` is RFC7526 deprecated 6to4-relay-anycast: `is_global=True` (guard allows the OAuth legs) yet special-purpose (kube-proxy-intercepted, **no real external egress**).
...
echo "{\"servers\":[\"${AS}/\"]}" | kubectl -n "$NS" exec -i deploy/vault -- sh -c 'cat > /tmp/as-allowlist.json'  # trailing slash: matches FastMCP's AnyHttpUrl-normalised PRM issuer
...
- [ ] **Step 3: Offline verification (NO kubectl/Vault/Docker/kind — the scripts RUN only at T9).** (a) `bash -n infra/proof-1b-2/seed-db.sh infra/proof-1b-2/seed-vault.sh` (syntax) → exit 0; `shellcheck` if available. (b) Add `tests/unit/proof_1b_2/test_proof_seeds.py` reading both scripts as text + asserting the invariant values: `T="proof-1b-2"`, the override `URL="http://10.96.0.50:8765/mcp"`, the allow-list `IP="10.96.0.50"`, `AS="http://192.88.99.9:9000"`, `ASHOST="192.88.99.9_9000"`; the table names (`mcp_server_url_override`, `mcp_internal_host_allowlist`); the Vault KV-v1 enable (`secrets enable -version=1 -path=secret kv`); and the `servers` JSON-LIST via the `@/tmp/as-allowlist.json` @file form (asserting `servers=` inline is NOT used); AND the AS allow-list entry carries the **trailing slash** (`${AS}/`) that the MCP server's pydantic-`AnyHttpUrl`-normalised PRM advertises — pinned by `test_vault_seed_allowlist_entry_carries_the_anyhttpurl_trailing_slash` (the kernel compares the issuer by exact string per RFC 8414 at `mcp_authz.py:753`, so the no-slash form silently fails the AS allow-list match — Proof 1b-2 attempt-4 finding). `uv run pytest tests/unit/proof_1b_2/test_proof_seeds.py -v` → passes.
...
- [ ] **Step 3: Runner** (`run-proof-1b-2.sh`, env-gated `COGNIC_RUN_PROOF_1B2=1`, exits 0 if unset). Sequence (extends `run-proof-1b-1.sh` verbatim where possible):
  ...
  - **BAR 1 (checkpoint) — pinned cold-cache sequence.** MCPHost caches BOTH the OAuth token and the list_tools result per tenant, so the allow-list-removed delta is only observable on a COLD pod (a rollout restart):
    1. (allow-list seeded) `curl -sf http://127.0.0.1:8000/api/v1/mcp/servers/cognic-tool-search/tools` → 200; assert via `psql` on the `audit_event` table (`SELECT payload::text WHERE event_type='audit.mcp_allowlist_permitted'`) that the permit fired carrying host `10.96.0.50` — the DD-2 audit event persists to the table, NOT pod stdout (attempt-5 finding). [Bar 1 positive]
    2. delete the allow-list row, restart to a cold pod, fresh probe MUST refuse:
       ...
       fresh `curl .../tools` MUST refuse — assert `mcp_discovery_url_refused` in the response body + `discovery_status=refused` via `/system/plugins` (the `refused_component=host_address` exception attr is not surfaced in the body/stdout — attempt-5 finding). [Bar 1 delta — the carve-out is load-bearing]
