...
**Files:** Create `infra/proof-1b-2/proof-1b-2-values.yaml`, `infra/proof-1b-2/migrate-job.yaml` (copy from 1b-1), `infra/proof-1b-2/run-proof-1b-2.sh`, `infra/proof-1b-2/README.md`
...
- [ ] **Step 5: Lint + commit** — `bash -n` the runner; `git add infra/proof-1b-2/{proof-1b-2-values.yaml,migrate-job.yaml,run-proof-1b-2.sh,README.md} && git commit -m "feat(proof-1b-2): values + migrate job + end-to-end runner (Bar 1 checkpoint + Bar 2 completion)"`
...
- [ ] Add a `proof-1b-2` job mirroring `kind-smoke`: `if: ${{ vars.COGNIC_RUN_PROOF_1B2 == '1' || github.event_name == 'workflow_dispatch' }}`, `helm/kind-action@v1` (`install_only: true`), step `run: bash infra/proof-1b-2/run-proof-1b-2.sh`. **Never default-on.** Commit `chore(proof-1b-2): env-gated kind CI job`.
