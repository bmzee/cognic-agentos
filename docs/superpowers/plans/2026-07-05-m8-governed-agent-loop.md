...
**External packs (Part B, separate repos):** `cognic-tool-oracle-schema` v0.3.0 (`run_readonly_query`); NEW `cognic-skill-customer-data`, `cognic-skill-financial-data`, `cognic-skill-atm-recon` (instruction-only); NEW `cognic-agent-bank-analyst`.
...
### Task B2: Three instruction-skill packs

**Repos:** NEW `bmzee/cognic-skill-customer-data`, `bmzee/cognic-skill-financial-data`, `bmzee/cognic-skill-atm-recon` (scaffold from `agentos init-skill`, then instruction-mode conversion).
...
- Manifest: `[pack] kind="agent"`; `[agent] persona_path="AGENT.md"`, `requested_skills=["customer-data","financial-data"]`, `requested_tools=["cognic-tool-oracle-schema/run_readonly_query"]`, `max_steps=6`; `[identity]` full agent-mandatory set (agent_card_url + agent_card_jws_path per `identity.py:300-335`); `[risk_tier] tier="customer_data_read"`; NO `[mcp]` block (A8 kind constraint).
...
- [ ] **Step 0 (bring-up, spec §6):** cluster + backends (proof-m6 flow); **oracle v0.3.0 through the full M4 operator lifecycle** (submit→review→approve→allow-list→install); agent + 3 skill packs **trust-registered at boot** (agent per-pack root live); oracle-seed + kernel-seed applied (0014 rows + proxy users + governed views); query-context keypair staged; assert `/api/v1/system/plugins` shows all 5 packs registered, `hosted_skills` lists the 3 instruction skills (+ the M6 executable skill posture unchanged), `hosted_agents` lists `bank-analyst`.
