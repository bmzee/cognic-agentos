...
#### B2-pre-2 (finding #3): sign/verify wheel-integrity instruction arm (kernel CC slice — maintainer-verified 2026-07-06)

**The finding (#3):** instruction packs are content packs (B2-pre made them discoverable + hostable), but the sign/verify wheel-integrity path stayed entry-point-anchored — `cli/_wheel_integrity.py` hard-failed a wheel with no `entry_points.txt` (`wheel_missing_entry_points_file`; live failure: `agentos sign --bundle .` on `cognic_skill_customer_data-0.1.0-py3-none-any.whl` → `sign_subprocess_failed`) and `cli/verify.py` Step 11 fail-closed on the empty validated entry-point tuple (`load_probe_no_validated_entry_points`). So B2's instruction packs could be discovered/hosted but never signed or verified.

**The ruling (verbatim constraints):** keep zero-executable instruction skills zero-executable (no inert marker, no `cognic.skills` entry point); `_wheel_integrity.py` may pass an absent `entry_points.txt` ONLY on exactly-one package-local `cognic-pack-manifest.toml` with `kind="skill"` + `[skill].mode="instruction"`; any other zero-entry-point wheel fails exactly as today; entry-point wheels byte/behavior unchanged; Step 11 still runs a FINAL isolated probe (module-import of the stub package — don't fake an entry point; `cli/_load_probe.py` gains a dedicated `probe_module_importability` path sharing the existing hardening); sign consumes the derived kind and signs instruction packs as skill packs with no AgentCard JWS; real built-wheel fixture tests; TM-revert pins for mode-loosen / kind-loosen / probe-skip / multi-manifest-allow.

- [x] **Implementation (TDD):** `tests/unit/cli/test_wheel_integrity_instruction.py` failing-first (44 tests: real PEP-427-complete instruction wheels — integrity happy path/legacy dual-path/METADATA anchoring; byte-pinned zero-manifest EXACT pre-fix failure; ambiguity/kind/mode/malformed/oversized/anti-decoy refusals; module-import probe arms incl. stdout-garbage + timeout + honest non-EntryPoint messages; reader lockstep drift detector vs `plugin_registry`); `_wheel_integrity.py` instruction fallback + 5-tuple return (5th slot = validated instruction package, None for entry-point wheels); `_load_probe.py` shared `_execute_probe_subprocess` core + explicit `probe_mode` argv marker; `verify.py` Step-11 instruction arm (fail-closed branch RETAINED for non-instruction empty tuples); `sign.py` 5-tuple consumption (JWS arm naturally excluded); e2e: `_stage_signed_instruction_pack` sign→verify PASS + probe spy + raising-`__init__` refusal (verify side) + sign-as-skill-no-JWS spy (sign side). TM-revert-proven: mode-loosen, kind-loosen, probe-skip (spy + behavioral), multi-manifest-allow.
- [x] **Docs:** ADR-002 "Sign/verify wheel-integrity instruction arm" paragraph; ADR-027 content-packs amendment one-line sign/verify extension; AGENTS.md `cli/_wheel_integrity.py` / `cli/verify.py` / `cli/_load_probe.py` entries (file:line verified).
- [ ] **Re-anchor sequencing:** the B2 external-pack arc re-anchors THIRD to the fix SHA (kernel dev-dep pin moves to the commit carrying this arm); `cognic-skill-customer-data` gets its anchor commit + ONE final tag move (`v0.1.0`) BEFORE rebuild/sign so the released wheel is signed by the fixed pipeline; the other three B2 packs anchor directly at the fixed SHA (no interim tags).
- [ ] **Gate:** full CC gate (149 files; `_wheel_integrity.py` / `verify.py` / `_load_probe.py` / `sign.py` ≥95/90 on fresh coverage) + `mcp_authz.py` byte-identical → controller review → **Commit** (halt for token).

#### B2-pre-3 (finding #4): AgentCard-JWS custody split (kernel CC slice — maintainer-verified 2026-07-06)

**The finding (#4):** the agent-pack AgentCard-JWS sign arm was never live-executable — surfaced by the FIRST real agent-pack sign (B3 `cognic-agent-bank-analyst`). (4a) `joserfc` lived only in the `adapters` extra → bare pack venv crashed `ModuleNotFoundError` at `sign.py`'s function-local import; (4b) the wrapper caught only `(OSError, RuntimeError, ValueError, TypeError)` vs its "wraps any exception" docstring → raw traceback, NO verdict line; (4c) the JWS arm read the SAME `signing_key_path` as cosign while needing an UNENCRYPTED RSA PEM (cosign needs the sigstore-encrypted format — different cryptographic identities through one path), and verify mirrored it (ONE `--trust-root` doing impossible double duty). Fixture-papers-over-production-gap instance #3 (after #1 discovery + #3 wheel-integrity).

**The ruling (Option A, verbatim constraints):** separate JWS custody (`Settings.agent_card_jws_signing_key_path` + `agent_card_jws_trust_root_path` + `--agent-card-trust-root`; `signing_key_path` cosign-only, NO fallback); tracked `agent-card.pub` for agent packs (committed pre-tag/release, release asset; verify JWS against IT, never `cosign.pub`); `joserfc == 1.6.4` → base `[project]` deps (also fixes the runtime `core/agent/query_context.py` RS256 packaging); 4b except tuple += `ImportError` → structured `sign_agent_card_jws_signing_failed`; tests incl. real separate-key sign/verify e2e, wrong-JWS-pub FAILS verify, missing-joserfc structured refusal, cosign-key-cannot-satisfy-JWS-path, in-toto expected set includes `agent_cards/agent-card.jws`, release assets include `agent-card.pub`.

- [x] **Implementation:** shared `_resolve_key_material_path` core + cosign-only/JWS-only wrappers (sign); shared `_resolve_trust_root_material` + `_resolve_agent_card_trust_root` w/ flag→setting→pack-root precedence + `agent_card_trust_root_unset` failure mode (verify); `JoseError`→`ValueError` translation + `ImportError` arm w/ `missing_module` payload; step 2b agent-only JWS-key resolution w/ manifest-kind peek; the two Settings + prod fixture-tree guard; `--agent-card-trust-root` CLI flag; joserfc→base; `init-agent` scaffold README + workflow document the custody split + assets; `tests/unit/cli/test_agent_card_jws_custody.py` + sign/verify/lifecycle e2e extensions. (Subagent stalled twice on infra; controller finished the batch inline: tool-pack staging fix to the house in-test-wheel pattern, Section-F prod-safe Settings kwargs, scaffold template docs.)
- [x] **Docs:** ADR-016 "AgentCard JWS custody split" amendment; ADR-027 cross-ref sentence; AGENTS.md `cli/sign.py` + `cli/verify.py` entries (file:line verified post-shift); this plan block.
- [ ] **B3 sequencing after the commit:** push kernel → re-anchor `cognic-agent-bank-analyst` to the fix SHA (B2's four released skill packs stay @e379dc2) → generate the RSA JWS keypair under custody (`~/.cognic/signing/cognic-agent-bank-analyst/v0.1.0/agent-card-rsa.{pem,pub}`) → track `agent-card.pub` in the pack → amend + ONE pre-release tag move → rebuild/sign/verify (real JWS replaces the placeholder) → Release w/ assets from the actual sign output.
- [ ] **Gate:** full CC gate (149 files; `sign.py` / `verify.py` ≥95/90 fresh) + `mcp_authz.py` byte-identical → controller review → **Commit** (halt for token).
