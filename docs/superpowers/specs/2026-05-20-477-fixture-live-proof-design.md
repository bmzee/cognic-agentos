...
#477 modifies **four existing test files** (none are created):
- `tests/conformance/sandbox/conftest.py` — the cross-backend conformance fixtures + the `_CANONICAL_SPRINT_8A_IMAGES` preflight.
- `tests/unit/sandbox/backends/conftest.py` — the per-backend env-gated unit-test fixtures + its own `_CANONICAL_SPRINT_8A_IMAGES` preflight.
