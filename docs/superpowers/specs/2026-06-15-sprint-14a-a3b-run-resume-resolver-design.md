...
4. `session = None`; teardown `finally: if session is not None: await session.destroy()` (resume **always** destroys on exit — no re-suspend in A3b).
...
6. wake succeeded → `transition(run_id, from_state="suspended", to_state="woken")` → `run.lifecycle.woken`. *(No direct executor event for `woken` — it is a mid-resume transition, not a request terminal; the store's lifecycle row captures it.)*
...
|---|---|---|
| `core/run/executor.py` | **CC** | run-record wiring, suspend path, conditional teardown, `resume()`, `RunRequest.suspend_after_exec`, `RunResult.run_id`, `RunTerminalState +suspended`, `RunNotResumable` |
