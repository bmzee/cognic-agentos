...
> **Plan-grounding correction (2026-06-21): the deferred JSON-RPC route-integration layer lands here too.** The `A2APolicyRefusalReason`/`A2AErrorCode` *literals* live in `protocol/__init__.py:463/426` (a2a_errors only imports + maps them), so the two new reasons are added there; the mapping + serializer live in `a2a_errors.py`. Because `handle()` *raises* `A2AEndpointError` and a2a_errors currently has **no envelope serializer**, a *correct* receiver requires building the "HTTP-route integration (deferred)" the module docstring (`:143`) names — in this slice: `from_endpoint_error(exc)`, a `to_jsonrpc(...)` serializer, a `_SPEC_CODE_TO_JSONRPC_INT` map (JSON-RPC 2.0 mandates an integer `error.code`), and a `_SPEC_CODE_TO_HTTP_STATUS` map. Kept **SDK-free**, with the integer-code map **drift-pinned against the `a2a-sdk` under `COGNIC_RUN_A2A_UPSTREAM=1`** (mirroring `a2a_schema.py`). The route catches `A2AEndpointError` → `from_endpoint_error` → `to_jsonrpc`; `handle()` stays focused on the method gate, not serialization. (A receiver that returns valid success bodies but invalid JSON-RPC error envelopes is not A2A-reachable in the sense a conformance test or a bank cares about — hence this is in-scope, even though it makes the slice less "thin.")
