...
- **Migration 0017:** composite index on `approval_requests (tenant_id, created_at, request_id)` — tenant-leading for the tenant-scoped WHERE; `(created_at, request_id)` is the KEYSET the pagination walks *(corrected 2026-07-11 during T1 planning: the original sentence named only the keyset pair)*. Guarded + re-runnable per the 0016 discipline (inspector-guarded DDL, shape validation, runtime-table parity pinned); live PostgreSQL + Oracle migration/query tests in the marker-gated lanes.
