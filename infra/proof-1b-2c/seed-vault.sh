#!/usr/bin/env bash
set -euo pipefail
NS="${NS:-cognic-proof1b2c}"; T="proof-1b-2c"; ASHOST="192.88.99.9_9000"; AS="http://192.88.99.9:9000"
# VAULT_TOKEN = the reused backends.yaml Vault dev root token (VAULT_DEV_ROOT_TOKEN_ID=smoke-root-token);
# must equal proof-1b-2c-values.yaml secrets.vaultToken (else `vault` 403s).
