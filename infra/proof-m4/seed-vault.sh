#!/usr/bin/env bash
set -euo pipefail
NS="${NS:-cognic-proofm4}"; T="proof-m4"; ASHOST="192.88.99.9_9000"; AS="http://192.88.99.9:9000"
# KEPT from proof-1b-2c (ADR-026 D5 — OAuth material provisioned BY REFERENCE): the
# operator pre-provisions the OAuth client + AS allow-list in Vault; the M4 `configure`
# step then records those Vault PATHS (oauth_credential_ref / as_allowlist_ref) on the
# desired runtime-config record, and `install`'s materializer VALIDATES they resolve +
# are well-shaped BEFORE it projects the derived carve-out rows. There is NO secret-write
# API — the operator seeds Vault out of band (this script).
#
# VAULT_TOKEN = the reused backends.yaml Vault dev root token (VAULT_DEV_ROOT_TOKEN_ID=smoke-root-token);
# must equal proof-m4-values.yaml secrets.vaultToken (else `vault` 403s).
