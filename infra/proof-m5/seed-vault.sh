#!/usr/bin/env bash
set -euo pipefail
NS="${NS:-cognic-proofm5}"; T="proof-m5"; ASHOST="192.88.99.9_9000"; AS="http://192.88.99.9:9000"
# KEPT from proof-m4 (ADR-026 D5 — OAuth material provisioned BY REFERENCE): the
...
# seeds Vault out of band (this script). The HOOK pack needs no Vault material:
# hooks are in-process kernel code (no OAuth, no MCP endpoint).
#
# VAULT_TOKEN = the reused backends.yaml Vault dev root token (VAULT_DEV_ROOT_TOKEN_ID=smoke-root-token);
# must equal proof-m5-values.yaml secrets.vaultToken (else `vault` 403s).
