"""PROOF-ONLY multi-actor app factory + header-driven binder for Proof M6.
...
module is the M6 mirror of ``tests/integration/proof_m5/proof_app.py`` — the
SAME shape (the two crux decisions below carry forward unchanged) with the M6
tenant + role identities and ONE scope delta — so the deployed
**governed-agent-skill proof** can drive the M4-proven operator-install flow
for ``cognic-tool-oracle-schema@v0.2.0`` and then the three governed-skill
bars against a real kernel.

**Deliberately NO skill-specific logic here.** The M6 delta is entirely
kernel-side: the ``create_app`` LIFESPAN constructs the managed sandbox
runtime (DockerSibling backend + scheduler control plane), then the governed
skill executor over the boot-trusted plugin registry + the MCP host + that
sandbox backend (``harness/skill_host.build_skill_executor``), and the boot
trust-registration resolves the skill pack's per-pack cosign trust root at the
LOCKED staged layout ``<trust_root_prefix>/skill-packs/<pack_id>/cosign.pub``
(``harness/registry_boot._SKILL_PACK_TRUST_ROOT_SUBDIR``). The skill pack is
trust-register + registry-admit + HOSTED only — it never enters the operator
lifecycle this app's routes serve. The ONE app-level delta vs M5: the ``mcp``
role additionally holds ``skill.invoke`` so the three bars can POST
``/api/v1/skills/schema-summary/invoke``.
...
still builds the runtime / MCP host / sandbox backend / skill executor / boot
registry on its OWN engine (the SAME Postgres via the same ``COGNIC_*`` DB
URL). **Two engines on one DB is acceptable for a PROOF-ONLY factory** —
production would inject ONE engine via a real bank-overlay deploy.
...
# ``frozenset[PackRBACScope]`` / ``frozenset[MCPRBACScope | SkillRBACScope]``
# IS assignable to the wider scope-union field. Same repo idiom as
# ``MCP_SCOPES`` (scopes.py).
# ---------------------------------------------------------------------------
...
#: against the released, signed pack), so the override cannot manufacture a
#: green signature; it only skips the four gates whose evidence this proof does
#: not attach. PROOF-ONLY (a real reviewer would attach genuine evaluation /
...
#: the operator a plausible examiner too). The override scope lives on the
#: REVIEWER role (the actor that hits ``/approve``), NOT here.
...
#: MCP + skill caller role — drives the governed MCP invoke surface AND the
#: governed skill invoke (POST /api/v1/skills/{skill_id}/invoke rides
#: ``skill.invoke`` per portal/rbac/scopes.py SkillRBACScope; ALL THREE M6
#: bars ride this role with the ARGUMENT as the only variable).
#: ``actor_type="service"`` (a machine principal), mirroring the proof-1b-2c
#: fixed-actor binder.
_MCP_SCOPES: Final[frozenset[MCPRBACScope | SkillRBACScope]] = frozenset(
    {"mcp.tool.list", "mcp.tool.invoke", "skill.invoke"}
)
...
class MultiActorProofBinder:
    ...
    :class:`Actor`. The 4 roles cover the full operator lifecycle plus the
    governed MCP + skill invoke surfaces the three M6 bars drive:
    ...
    - ``mcp``      — ``mcp.tool.{list,invoke}`` + ``skill.invoke``,
      ``actor_type="service"`` (the governed MCP invoke route + the governed
      skill invoke route).
    ...
    def bind(self, *, request: Request | None) -> Actor:  # matches the kernel ActorBinder Protocol
        ...
        if factory is None:
            raise UnknownProofRole(
                f"proof-m6: no proof actor for {PROOF_ROLE_HEADER}={role!r}; "
...
def create_proof_app() -> FastAPI:
    """Build the PROOF-ONLY multi-actor app for the M6 governed-skill proof.
    ...
    **Eager-injection wiring (Key Decision A).** Deferred imports keep this
    ...
    (mirrors ``proof_m5.proof_app``). The factory:
    ...
    3. builds a real :class:`TrustGate` + a proof-only
       ...
       ``v0.2.0`` pack against the staged ``_default`` trust root);
    ...
    managed sandbox runtime (scheduler control plane -> DockerSibling backend)
    and the governed skill executor over that boot-trusted registry + the MCP
    host (``build_skill_executor`` -> ``app.state.skill_executor`` +
    ``app.state.hosted_skills``) — no skill-specific wiring lives in this
    factory. The lifespan builds its OWN runtime (its own engine, same DB); its
    ``app.state.runtime_config_store`` / ``runtime_config_materializer``
    ...
    if not settings.database_url:
        raise RuntimeError(
            "proof-m6: settings.database_url is unset; the eager operator-store "
    ...
    # 5-gate's SIGNATURE gate resolves GENUINELY (the released v0.2.0 pack is
    ...
    # runtime + the skill executor + the MCP host on its own engine.
    app = create_app(
        settings,
...
class ProofStagedTrustRootResolver:
    ...
    image (``COPY proof-m6-staging/trust-roots/
    ...
    trust-registration uses (the released ORACLE pack's ``cosign.pub``), so the
    approve 5-gate's signature gate cosign-verifies the released, signed
    ``v0.2.0`` pack against a REAL trust root. The HOOK + SKILL packs'
    DIFFERENT signer keys are staged per-pack at
    ``trust-roots/hook-packs/<pack_id>/cosign.pub`` and
    ``trust-roots/skill-packs/<pack_id>/cosign.pub`` and consumed by the
    kernel's boot loop (``registry_boot``), NOT by this resolver — neither pack
    ever enters the approve flow (trust-register + registry-admit only).
