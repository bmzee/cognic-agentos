#!/usr/bin/env bash
set -euo pipefail
NS="${NS:-cognic-proofm6}"; T="proof-m6"; ASHOST="192.88.99.9_9000"; AS="http://192.88.99.9:9000"
# KEPT from proof-m4/m5 (ADR-026 D5 — OAuth material provisioned BY REFERENCE): the
# operator pre-provisions the OAuth client + AS allow-list in Vault; the M4-flow
# `configure` step then records those Vault PATHS (oauth_credential_ref /
# as_allowlist_ref) on the desired runtime-config record for the v0.2.0 tool pack,
...
# seeds Vault out of band (this script). Neither the HOOK pack nor the SKILL pack
# needs Vault material: hooks are in-process kernel code, and the sandboxed skill
# action runs with requires_credentials=() — NO ambient credentials by design
# (spec §5.4 invariant #10); its tool calls acquire OAuth material KERNEL-side
# through the broker -> MCPHost path, from these same by-reference seeds.
#
# VAULT_TOKEN = the reused backends.yaml Vault dev root token (VAULT_DEV_ROOT_TOKEN_ID=smoke-root-token);
# must equal proof-m6-values.yaml secrets.vaultToken (else `vault` 403s).
