...
# M8 kernel-side DB seed — TWO contracts in one place (drift-prevention: the
...
#
# 2. NEW AT M8 (ADR-027, migration 0014): apply kernel-seed.sql — the
...
#    (ON CONFLICT DO NOTHING) + tenant-scoped to proof-m8. Applied AFTER the
#    migration Job brought the schema to rev 0014; readback-asserted here so
#    a silent partial apply can never reach the bars.
#
...
echo "seed-db.sh (M8): applying kernel-seed.sql (0014 rows; tenant=$T, ns=$NS)"
...
if [ "$COUNTS" != "4|4|4|0" ]; then
  echo "FAIL: seed-db.sh (M8) readback expected 4|4|4|0 (scopes|entitlements|assignments|atm_recon-entitlements), got: $COUNTS" >&2
  exit 1
fi
echo "seed-db.sh (M8): 0014 rows verified (4 scopes, 4 entitlements, 4 assignments, atm_recon entitled to NOBODY)"
