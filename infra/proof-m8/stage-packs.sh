#!/usr/bin/env bash
# infra/proof-m8/stage-packs.sh — stage the released, signed packs for the
# M8 (governed agent loop) proof. RELEASED ASSETS ONLY (never a source rebuild):
#
...
# staging tree the Dockerfiles consume). run-proof-m8.sh (Task C2) calls this
# at its stage step:  bash infra/proof-m8/stage-packs.sh <staging-dst>
#
...
# and BEFORE the image builds (proof-m8-staging/query-context/ carries ONLY
...
STAGING_DST="${1:-$REPO_ROOT/infra/proof-m8/proof-m8-staging}"
