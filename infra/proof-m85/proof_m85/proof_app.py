"""PROOF-ONLY multi-actor app factory + header-driven binder for the Proof
M8.5 SLICE (ADR-028 conversational substrate, BARs 1-3).
...
module is the M8.5 mirror of the proof-m8 ``proof_app.py`` — the
SAME shape (the two crux decisions below carry forward unchanged) with the
M8.5 tenant + role identities and the ONE M8.5 delta:

* **The two ANALYST actors** — ``analyst.amir`` + ``analyst.sara`` — carry
  ONLY the four ``conversation.*`` scopes
  (``portal/rbac/scopes.py::ConversationRBACScope``) and deliberately NO
  ``agent.ask`` (maintainer ruling 2026-07-10): the M8.5 slice exercises the
  CONVERSATION surface exclusively, and a stray single-shot ``/ask`` from an
  analyst must 403 rather than silently bypass the conversation record. The
  kernel-side entitlement matrix (migration-0014 ``entitlements`` rows,
  seeded by ``kernel-seed.sql``) keys on EXACTLY these ``Actor.subject``
  strings — amir is entitled to ``retail_analytics + financials``, sara to
  ``cards_analytics + retail_analytics`` — and the A10 dispatch chokepoint
  reads the subject off the bound Actor (threaded per turn by the
  ConversationTurnExecutor, never the request body), so BAR 3's live
  revocation is enforced kernel-side per request identity per turn.
* The ``mcp`` role keeps the two governed MCP scopes for the SETUP warm-up
  ``list_tools`` probe (the M8 posture, unchanged).

**Deliberately NO conversation-specific logic here.** The M8.5 delta is
entirely kernel-side: the ``create_app`` LIFESPAN constructs the managed
sandbox runtime + the skill executor (the M6 posture, unchanged — the
hosted_skills surface Step 0 asserts), the governed agent loop
...
(``COGNIC_AGENT_QUERY_CONTEXT_SIGNING_KEY_PATH``), and THEN the ADR-028
conversation substrate (``ConversationStore`` + ``ConversationTurnExecutor``
gated on ``app.state.agent_loop``; the runner sets
``COGNIC_CONVERSATION_CLAIM_TTL_S=600`` so the executor's
claim-TTL-exceeds-wall-clock construction guard passes).
...
# ``frozenset[ConversationRBACScope]`` IS assignable to the wider scope-union
# field. Same repo idiom as ``MCP_SCOPES`` / ``CONVERSATION_SCOPES`` (scopes.py).
# ---------------------------------------------------------------------------
...
#: Analyst roles — the M8.5 bars' conversing identities. ONLY the four
#: ``conversation.*`` scopes and deliberately NO ``agent.ask`` (maintainer
#: ruling 2026-07-10): an analyst can create / read / drive / close governed
#: CONVERSATIONS and nothing else — no single-shot ``/ask`` (403), no pack
#: lifecycle, no raw MCP invoke. The kernel-side entitlement matrix keys on
#: the SUBJECT strings below; the two Actors differ ONLY by subject.
_ANALYST_SCOPES: Final[frozenset[ConversationRBACScope]] = frozenset(
    {
        "conversation.create",
        "conversation.read",
    ...
    def bind(self, *, request: Request | None) -> Actor:  # matches the kernel ActorBinder Protocol
        ...
        if factory is None:
            raise UnknownProofRole(
                f"proof-m85: no proof actor for {PROOF_ROLE_HEADER}={role!r}; "
...
def create_proof_app() -> FastAPI:
    """Build the PROOF-ONLY multi-actor app for the M8.5 conversational-slice proof.
    ...
    **Eager-injection wiring (Key Decision A).** Deferred imports keep this
    ...
    if not settings.database_url:
        raise RuntimeError(
            "proof-m85: settings.database_url is unset; the eager operator-store "
...
class ProofStagedTrustRootResolver:
    ...
    image (``COPY proof-m85-staging/trust-roots/
