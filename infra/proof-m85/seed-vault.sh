#!/usr/bin/env bash
set -euo pipefail
NS="${NS:-cognic-proofm85}"; T="proof-m85"; ASHOST="192.88.99.9_9000"; AS="http://192.88.99.9:9000"
...
# Secret mount (see run-proof-m85.sh + agentos-sandbox-patch.yaml; vault://
...
# must equal proof-m85-values.yaml secrets.vaultToken (else `vault` 403s).
...
# secret/cognic/proof-m85/litellm (run-proof-m85.sh step 8). The field name MUST
