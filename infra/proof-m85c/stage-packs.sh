...
# staging tree the Dockerfiles consume). run-proof-m85c.sh (Task C2) calls this
...
# and BEFORE the image builds (proof-m85c-staging/query-context/ carries ONLY
...
STAGING_DST="${1:-$REPO_ROOT/infra/proof-m85c/proof-m85c-staging}"
...
# 5-gate signature root). NOTE: the probe's RELEASED cosign.sig is signed with
# the probe's OWN key; the runner's re-sign step (run-proof-m85c.sh, after this
# script) overrides trust-roots/_default/cosign.pub with a proof approve key and
# re-signs BOTH tools-kind wheels (oracle + probe) under it, because the
# tenant-keyed approve trust root cannot carry two release keys. See the runner.
if [ "${COGNIC_PROOF_M85C_PROBE_RELEASED:-0}" = "1" ]; then
  PROBE_SRC="$TMP/probe"
  _download_release "$PROBE_REPO" "$PROBE_TAG" "$PROBE_SRC"
  [ "$PROBE_WHEEL_SHA256" != "FILL_AT_RELEASE" ] || die "the probe release is enabled but its WHEEL digest is still the FILL_AT_RELEASE sentinel. A MAINTAINER must COMMIT the real sha256 (printed by the probe's release.sh) into stage-packs.sh before the live run — it is deliberately NOT an environment variable, because a pin the operator supplies at run time is not a pin at all."
  [ "$PROBE_PUB_SHA256" != "FILL_AT_RELEASE" ] || die "the probe release is enabled but its cosign.pub digest is still the FILL_AT_RELEASE sentinel. A MAINTAINER must COMMIT the real sha256 into stage-packs.sh before the live run (see the wheel-digest message above)."
  _verify_digest "$PROBE_SRC/$PROBE_WHEEL" "$PROBE_WHEEL_SHA256" "$PROBE_PACK_ID wheel"
  _verify_digest "$PROBE_SRC/cosign.pub" "$PROBE_PUB_SHA256" "$PROBE_PACK_ID cosign.pub"
  cp "$PROBE_SRC/$PROBE_WHEEL" "$STAGING_DST/wheel/$PROBE_WHEEL"
  _stage_pack_attestations "$PROBE_SRC" "$PROBE_PACK_ID" "$PROBE_VERSION" "$PROBE_WHEEL"
  # Durable copy of the probe's RELEASED cosign.pub (see the release-pubs note by
  # the oracle staging) so the runner can verify the probe's original signature
  # before re-signing it under the proof approve key.
  mkdir -p "$STAGING_DST/release-pubs"
  cp "$PROBE_SRC/cosign.pub" "$STAGING_DST/release-pubs/$PROBE_PACK_ID.pub"
...
python3 - "$STAGING_DST/policies/plugin_allowlist.json" "${_ALLOWLIST_PACKS[@]}" <<'PY'
import json, sys
out, packs = sys.argv[1], sys.argv[2:]
with open(out, "w", encoding="utf-8") as handle:
    json.dump({"_default": packs}, handle)
    handle.write("\n")
PY
echo "  plugin allow-list: ${#_ALLOWLIST_PACKS[@]} packs admitted for _default"
