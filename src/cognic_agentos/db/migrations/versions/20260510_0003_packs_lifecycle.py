...
- ``sa.Uuid()`` for the primary key — the dialect-portable seam introduced
  ...
  and the in-process Table at ``packs/storage.py:200`` — the migration
...
Pydantic model at ``packs/storage.py:352-379`` are caught a second time
...
``packs/storage.py:200`` and this migration's ``op.create_table(...)``
...
def upgrade() -> None:
    op.create_table(
        ...
        # at ``packs/storage.py:216-225``. Constraint names are pinned
    ...
    # Index inventory matches ``packs/storage.py:226-227``.
