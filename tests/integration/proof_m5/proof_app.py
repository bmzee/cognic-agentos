"""PROOF-ONLY multi-actor app factory + header-driven binder for Proof M5.
...
module is the M5 mirror of ``tests/integration/proof_m4/proof_app.py`` — the
SAME shape (the two crux decisions below carry forward unchanged) with the M5
tenant + role identities — so the deployed **hook-pack DLP proof** can drive
the M4-proven operator-install flow for ``cognic-tool-oracle-schema@v0.2.0``
and then the three DLP bars against a real kernel.

**Deliberately NO hook-specific logic here.** The M5 DLP delta is entirely
kernel-side: the ``create_app`` LIFESPAN builds the ``DLPGuard`` over the same
boot-trusted plugin registry (``harness/hook_registry.build_dlp_guard``) and
threads it into the MCP host, and the boot trust-registration resolves the
hook pack's per-pack cosign trust root at the LOCKED staged layout
``<trust_root_prefix>/hook-packs/<pack_id>/cosign.pub``
(``harness/registry_boot._resolve_pack_trust_root``, ADR-002 hooks amendment).
The hook pack is trust-register + registry-admit ONLY (spec §6 decision B) —
it never enters the operator lifecycle this app's routes serve.
...
#: chain (the DLP-evidence assertions read the DB directly, but the scope keeps
...
#: MCP caller role — drives the governed MCP invoke route (list + call; ALL
#: THREE M5 DLP bars ride this role with the argument as the only variable).
...
class MultiActorProofBinder:
    ...
    governed MCP invoke surface the three DLP bars drive:
    ...
    def bind(self, *, request: Request | None) -> Actor:  # matches the kernel ActorBinder Protocol
        ...
        if factory is None:
            raise UnknownProofRole(
                f"proof-m5: no proof actor for {PROOF_ROLE_HEADER}={role!r}; "
...
def create_proof_app() -> FastAPI:
    """Build the PROOF-ONLY multi-actor app for the M5 hook-pack DLP proof.
    ...
    **Eager-injection wiring (Key Decision A).** Deferred imports keep this
    ...
    (mirrors ``proof_m4.proof_app``). The factory:
    ...
    and install gate 2 reads it per-request. The SAME lifespan builds the M5
    ``DLPGuard`` over that boot-trusted registry and threads it into the MCP
    host (``build_dlp_guard`` + ``build_mcp_host(dlp_guard=...)``) — no
    hook-specific wiring lives in this factory. The lifespan builds its OWN
    runtime (its own engine, same DB); its ``app.state.runtime_config_store`` /
    ...
    if not settings.database_url:
        raise RuntimeError(
            "proof-m5: settings.database_url is unset; the eager operator-store "
    ...
    # lifespan builds the boot registry (request-time gate 2) + the DLPGuard +
    # the MCP host on its own engine.
    app = create_app(
        settings,
...
class ProofStagedTrustRootResolver:
    ...
    image (``COPY proof-m5-staging/trust-roots/
    ...
    ``v0.2.0`` pack against a REAL trust root. The HOOK pack's DIFFERENT signer
    key is staged per-pack at ``trust-roots/hook-packs/<pack_id>/cosign.pub``
    and consumed by the kernel's boot loop (``registry_boot``), NOT by this
    resolver — the hook pack never enters the approve flow (trust-register +
    registry-admit only, spec §6 decision B). Production injects a real
    per-tenant Vault-backed resolver (``secret/cognic/<tenant>/trust-root`` per
    ADR-012 §134); the kernel default is fail-loud. This proof-only resolver
    returns a real staged path so the signature gate is GENUINELY exercised
    (NOT stubbed).
    """
