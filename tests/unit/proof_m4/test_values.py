"""Structural pin — the Helm values overlay + the non-hook migrate Job (M4 Task 8)."""

from pathlib import Path

import yaml

V = yaml.safe_load(Path("infra/proof-m4/proof-m4-values.yaml").read_text())
MJ = Path("infra/proof-m4/migrate-job.yaml").read_text()
...
def test_values_vault_token_matches_seed_vault():
    ...
    assert "VAULT_TOKEN=smoke-root-token" in Path("infra/proof-m4/seed-vault.sh").read_text()
