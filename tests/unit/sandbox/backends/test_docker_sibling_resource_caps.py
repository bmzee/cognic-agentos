"""Sprint 8A T10b — env-gated resource-cap enforcement against real cgroups.

ENV-GATED: skipped unless ``COGNIC_RUN_DOCKER_SANDBOX=1`` AND a
Docker daemon + cgroups are reachable. Standard pytest runs skip
these tests; local dev + the Sprint-8A sandbox-integration CI lane
runs them.

Re-uses the ``backend`` + ``catalog`` + ``docker_client`` +
``_canonical_artifact_preflight`` fixtures from
``test_docker_sibling_lifecycle.py`` via the conftest-shadow
auto-discovery pytest does on the same directory.

Per spec §7 + round-3 P2 invariant: ``--cpus`` throttling under cap
is NOT a violation by itself; only ``cpu_time_budget_exceeded`` fires
(and only when ``policy.cpu_time_budget_s`` is set).
"""
...
pytestmark = pytest.mark.skipif(
    ...
    reason="Docker daemon + cgroups required — set COGNIC_RUN_DOCKER_SANDBOX=1 to run",
)
...
class TestResourceCapsFireOrDontFire:
    ...
    async def test_memory_oom_emits_memory_cap_exceeded(
        ...
        kills via OOM-killer (exit 137 + State.OOMKilled=True);
        backend raises SandboxPolicyViolated(memory_cap_exceeded)."""
    ...
    async def test_cpu_time_budget_exceeded_fires_when_budget_set(
        ...
        """Per spec §7 item 4: container.stats CPU-usage polled at ≥1Hz;
